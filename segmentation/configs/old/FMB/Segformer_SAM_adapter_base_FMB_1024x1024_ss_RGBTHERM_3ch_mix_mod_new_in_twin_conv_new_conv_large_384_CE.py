...
_base_ = [
    ...
    '../_base_/datasets/FMB.py', #DELIVER_MM.py
...
# crop_size =(896,896)
# stride=(368,200)
# mod_dir_tr='samples/depth/training',
# mod_dir_val='samples/depth/validation',
# mod_dir_test='samples/depth/test',
# mod_suffix='_depth_front.png',
stride=(240,240)
img_scale = (800,600)
...
pretrained = 'pretrained/sam_vit_b_image_encoder_no_neck.pth'#/media/data2/icurti/projects/sina/AI_server_client/ViTAdapter/segmentation/pretrained/sam_vit_b_01ec64.pth
norm_cfg = dict(type='SyncBN', requires_grad=True)

model = dict(
    # type='EncoderDecoder',
    # type='EncoderDecoderDebug',
    ...
    backbone=dict( 
        _delete_=True,
        type='SAMAdapterbimodalMixModNewInTwinConvNEWwithcp',#withcp
        img_size=crop_size[0],
        ...
        arch='large',
        checkpoint=
        "https://download.openmmlab.com/mmclassification/v0/convnext/convnext-large_in21k-pre-3rdparty_in1k-384px_20221219-6d38dd66.pth"
    ...
    # test_cfg=dict(mode='slide',rescale=True, dim=(1024,1024))
    # test_cfg=dict(mode='slide_mod_sel', crop_size=crop_size, stride=(240, 240))
    # test_cfg=dict(mode='whole_dim',rescale=True, dim=(480,640))
    test_cfg=dict(mode='whole_dim_cut',rescale=False, dim=(800,800),cut_dim=(800,600))
)
# dataset settings
...
train_pipeline = [
    ...
    dict(type='Resize_multimodal', img_scale=crop_size,modalities_name=modalities_name, modalities_ch=modalities_ch),
    dict(type='Resize_multimodal', img_scale=crop_size, ratio_range=(0.5, 2.0),modalities_name=modalities_name, modalities_ch=modalities_ch),
...
test_pipeline = [
    ...
    dict(type='Resize_multimodal', img_scale=crop_size,modalities_name=modalities_name, modalities_ch=modalities_ch),
    dict(type='Pad_multimodal', size=crop_size, pad_val=0, seg_pad_val=255),
    # dict(type='Resize_multimodal', img_scale=crop_size, seg_scale=(640,480),keep_ratio=False,modalities_name=modalities_name, modalities_ch=modalities_ch),
    dict(
...
data = dict(samples_per_gpu=2,
            ...
            test=dict(pipeline=test_pipeline, modalities_name=modalities_name, modalities_ch=modalities_ch))
...
evaluation = dict(start=1, interval=1, by_epoch=True, metric='mIoU', save_best='mIoU', resize_dim=(800,800), case=None)#['motionblur', 'overexposure', 'underexposure', 'lidarjitter', 'eventlowres'])#, show=True), out_dir="/media/data4/sora/projects/ViT-Adapter/segmentation/training_DELIVER_dataset_RGB_SAMADAPTER_1024x1024/test")#by_epoch=True, _real_train _deliver_pipeline
