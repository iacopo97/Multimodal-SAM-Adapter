...
_base_ = [
    ...
    '../_base_/datasets/DELIVER_RGB.py',
...
model = dict(
    ...
    # test_cfg=dict(mode='slide_pipeline1', crop_size=(965,965), stride=stride)
...
train_pipeline = [
    ...
    dict(type='Normalize', **mod_norm_cfg),
...
test_pipeline = [
    ...
    dict(type='Resize_4_test', img_scale=crop_size, seg_scale=(1024,1024),keep_ratio=True),
    dict(
        ...
        transforms=[
            ...
            # dict(type='Collectmod', keys=['img'],modalities_name=['rgb','depth','event','lidar'],modalities_ch=[3,3,3,1]),
...
# optimizer_config = dict(type="GradientCumulativeOptimizerHook", cumulative_iters=4) #maybe 8
optimizer_config = dict(type="GradientCumulativeFp16OptimizerHook", cumulative_iters=8) #maybe 8,16
...
evaluation = dict(start=1, interval=1, by_epoch=True, metric='mIoU', save_best='mIoU', resize_dim=(1024,1024))
#show=True, out_dir="/media/data4/sora/projects/ViT-Adapter/segmentation/training_DELIVER_dataset_RGB_BEITADAPTER_batch8/test")#by_epoch=True, _real_train _deliver_pipeline