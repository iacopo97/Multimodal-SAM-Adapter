...
model = dict(
    ...
    backbone=dict( 
        _delete_=True,
        type='SAMAdaptermultimodalselectorgamma_nogammafinalonebranch',
        img_size=crop_size[0],
...
optimizer = dict(_delete_=True, type='AdamW', lr=2e-5, betas=(0.9, 0.999), weight_decay=0.05,#lr=2e-5 6e-5,weight_decay=0.05 0.01 lr=2e-4 for GEMINI Fusion6
                 constructor='LayerDecayOptimizerConstructor',#'DefaultOptimizerConstructor'
                 paramwise_cfg=dict(num_layers=24, layer_decay_rate=0.90))
lr_config = dict(_delete_=True,
                 policy='poly',
                 warmup='linear',#original'linear' 'exp'
                 warmup_iters=1500,#1500, 10
                 warmup_ratio=1e-6,#1e-6 0.1
                 power=1.0,#1.0, 0.9
                 min_lr=0.0, by_epoch=False) #False
