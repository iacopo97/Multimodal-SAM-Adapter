        ...
        for pred, index in zip(preds, indices):
            ...
            pre_eval_results.append(
                intersect_and_union(
                    ...
                    label_map=dict(),
                    reduce_zero_label=False))
            # Sum the intersections and unions
            # self.total_intersect += pre_eval_results[0][0].sum()
            # self.total_union += pre_eval_results[0][1].sum()
        # # Compute micro IoU
        # micro_iou = total_intersect / total_union if total_union > 0 else 0
        ...
        for keys_ext in ret_metrics.keys():
            ...
            if metric[0]=='microIoU':
                if keys_ext!='global':   
                    ...
                    # ret_metrics_summary_condition=OrderedDict({
                    #     condition: np.round(np.nanmean(np.array(ret_metrics[keys_ext][condition])) * 100,2)
                    #     for condition in ret_metrics[keys_ext].keys()
                    #     if condition in ['ordinary','motionblur','overexposure','underexposure','eventlowres','lidarjitter']
                    # })
                    # each class table
