...
class PST900(CustomDataset):
    ...
    PALETTE = [[64,0,128],[64,64,0],[0,128,192],[0,0,192],[128,128,0],[64,64,128],[192,128,128],[192,64,0]]
    ...
    def load_annotations_modalities(self, img_dir, img_suffix,mod_dir_dict, mod_suffix_dict,modalities_name, ann_dir, seg_map_suffix,
        ...
        if split is not None:
            ...
            if split == 'test':
                source = osp.join(self.data_root, 'test.txt')
            elif split == 'val':
                source = osp.join(self.data_root, 'val.txt')
            elif split == 'train':
                source = osp.join(self.data_root, 'train_without_flip.txt')
            with open(source) as f:
                files=f.readlines()
                # img_names =[]
                for line in files:
