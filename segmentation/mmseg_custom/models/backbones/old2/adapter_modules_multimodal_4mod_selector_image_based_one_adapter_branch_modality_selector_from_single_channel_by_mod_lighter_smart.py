...
class SpatialPriorModuleModSelectorsinglechannel(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3):
        ...
        self.fc_mod3 = nn.Conv2d(4 * inplanes, embed_dim//4, kernel_size=1, stride=1, padding=0, bias=True) #needed for creating the spatial gamma for injector
        self.fc_mod4 = nn.Conv2d(4 * inplanes, embed_dim//4, kernel_size=1, stride=1, padding=0, bias=True)
        self.mod_selector = ModalitySelector(embed_dim=embed_dim//4, with_cp=with_cp,num_mod=1)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            c3 = self.conv3(c2)
            c4 = self.conv4(c3)
            mod1=self.fc_mod1(c1)
            mod2=self.fc_mod2(c2)
            mod3=self.fc_mod3(c3)
            mod4=self.fc_mod4(c4)
            c1 = self.fc1(c1)
            c2 = self.fc2(c2)
            c3 = self.fc3(c3)
            c4 = self.fc4(c4)
            mod_selector_gamma=self.mod_selector(mod1,mod2,mod3,mod4)
            bs, dim, _, _ = c1.shape
            c1 = c1.view(bs, dim, -1).transpose(1, 2)  # 4s 2,1024, HW/4^2 view is flattening the tensor transpose therefore 2, HW/4^2, 1024
            c2 = c2.view(bs, dim, -1).transpose(1, 2)  # 8s
            c3 = c3.view(bs, dim, -1).transpose(1, 2)  # 16s
...
class ModalitySelectorViT(nn.Module):
    ...
    def forward(self, x,c1_orig_shape,c3_orig_shape):
        ...
        def _inner_forward(x):
            ...
            mod_selector_gamma=F.interpolate(mod_selector_gamma3, size=c1_orig_shape[-2:], mode='bilinear', align_corners=False)
            # mod_selector_gamma4=F.interpolate(mod_selector_gamma3, size=c4_orig_shape[-2:], mode='bilinear', align_corners=False)            
            # mod_selector_gamma1=mod_selector_gamma1.view(bs, self.num_mod, -1).transpose(1, 2)  # 4s bs,num_mod, HW/4^2 view is flattening the tensor transpose therefore bs, HW/4^2, num_mod
            # mod_selector_gamma2=mod_selector_gamma2.view(bs, self.num_mod, -1).transpose(1, 2) #8s
            # mod_selector_gamma3=mod_selector_gamma3.view(bs, self.num_mod, -1).transpose(1, 2) #16s
            # mod_selector_gamma4=mod_selector_gamma4.view(bs, self.num_mod, -1).transpose(1, 2) #32s
            return mod_selector_gamma
...
class ModalityMerger(nn.Module):
    def __init__(self, embed_dim=64, with_cp=False, num_mod=4):
        super().__init__()
        self.with_cp = with_cp
        self.num_mod = num_mod
    ...
    def forward(self, x,c1_orig_shape,c2_orig_shape,c3_orig_shape,c4_orig_shape):
