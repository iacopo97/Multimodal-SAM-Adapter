...
from .adapter_modules_multimodal_1mod_selector_image_based_one_adapter_branch import SpatialPriorModule, SpatialPriorModulewithGammaSpatial, InteractionBlock, deform_inputs
...
class SAMAdaptermultimodal1modselectoronebranchRelunormconcat(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        self.merger_1.apply(self._init_weights)
        self.merger_2_4.apply(self._init_weights)
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            ...
            # mod_selector_gamma1=F.normalize(mod_selector_gamma1, p=1, dim=1)
            # mod_selector_gamma2=F.normalize(mod_selector_gamma2, p=1, dim=1)
            # mod_selector_gamma3=F.normalize(mod_selector_gamma3, p=1, dim=1)
            # mod_selector_gamma4=F.normalize(mod_selector_gamma4, p=1, dim=1)
        ...
        if self.num_mod>1:
            ...
            c1=torch.cat([c1,c1_other_modalities_cumulative],dim=2)
            c=torch.cat([c,c2_4_other_modalities_cumulative],dim=2)
            c1=self.merger_1(c1)
            c=self.merger_2_4(c) 
