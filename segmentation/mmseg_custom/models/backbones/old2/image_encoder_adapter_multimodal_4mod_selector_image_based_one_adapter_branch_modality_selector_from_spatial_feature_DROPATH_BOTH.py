...
class SAMAdaptermultimodal4modselectoronebranchModFromSpDROPATHBOTH(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        # self.multimodal_drop_path2=DropPath(drop_multimodal_path) if drop_multimodal_path > 0. else nn.Identity()
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            ...
            # c1, c2, c3, c4, mod_selector_gamma1, mod_selector_gamma2, mod_selector_gamma3, mod_selector_gamma4 = self.spm_gamma_spatial(x)
            mod_selector_gamma1,mod_selector_gamma2,mod_selector_gamma3,mod_selector_gamma4,c1,c2,c3,c4=self.spm_mod_selector(self.multimodal_drop_path(x))
        ...
        if self.num_mod>1:
            # SPM forward
            for i in range(1,len(self.modalities_name)):
                c1_other_modalities, c2_other_modalities, c3_other_modalities, c4_other_modalities = self.spm_other_modalities[f"spm_{self.modalities_name[i]}"](self.multimodal_drop_path(x_other_modalities[:,modalities_ch_cumsum[i-1]:modalities_ch_cumsum[i]]))
