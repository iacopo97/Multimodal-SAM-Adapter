...
from .base.image_encoder import ImageEncoderViT,PatchEmbed
...
class SAMAdapterbimodalconcatnoRELUpatchembedmod(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        if self.num_mod>1:
            self.patch_embed_other_mod = PatchEmbed(
                kernel_size=(16, 16),
                stride=(16, 16),
        ...
        self.merger_patch_embed=nn.Linear(embed_dim*2,embed_dim)
        ...
        self.merger_patch_embed.apply(self._init_weights)
        ...
        if self.num_mod>1:
            ...
            self.patch_embed_other_mod.apply(self._init_weights)
    ...
    def forward(self, x):
        ...
        x_other_modalities, _, _ = self.patch_embed_other_mod(x_other_modalities)
        x=self.merger_patch_embed(torch.cat([x,x_other_modalities],dim=2))
        # x=x.flatten(2)
        # x=x.flatten(1,2)
        bs, n, dim = x.shape
