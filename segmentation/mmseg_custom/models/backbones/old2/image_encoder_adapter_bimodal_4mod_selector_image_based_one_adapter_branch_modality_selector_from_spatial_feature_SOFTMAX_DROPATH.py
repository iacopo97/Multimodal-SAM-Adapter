...
class SAMAdapterbimodal4modselectoronebranchModFromSpSOFTMAXDROPATH(ImageEncoderViT):
    ...
    def _init_weights(self, m):
        ...
        elif (isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d)):# and ((m.in_channels!=4*self.embed_dim) or (m.out_channels!=self.num_mod)):
            fan_out = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
            fan_out //= m.groups
            m.weight.data.normal_(0, math.sqrt(2.0 / fan_out))
            if m.bias is not None:
                m.bias.data.zero_()
        # elif isinstance(m, nn.Conv2d) and (m.in_channels==4*self.embed_dim) and (m.out_channels==self.num_mod):
        #     # Use Xavier initialization for convolutional layers
    ...
    def forward(self, x):
        ...
        if self.num_mod>1:
            # SPM forward
            c1_other_modalities, c2_other_modalities, c3_other_modalities, c4_other_modalities = self.multimodal_drop_path(self.spm_other_modalities(x_other_modalities))
