...
from .adapter_modules_multimodal_4mod_selector_image_based_one_adapter_branch import SpatialPriorModule, InteractionBlock, deform_inputs, ModalitySelector
...
class SAMAdaptermultimodal4modselectoronebranchRelunormseparated(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        if ('rgb' in self.modalities_name):
            ...
            self.mod_selector=ModalitySelector(inplanes=conv_inplane, with_cp=False, in_channels=self.in_ch_im,num_mod=self.num_mod)
            ##INITIALIZE THEM###
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            ...
            mod_selector_gamma1, mod_selector_gamma2, mod_selector_gamma3, mod_selector_gamma4=self.mod_selector(x)
