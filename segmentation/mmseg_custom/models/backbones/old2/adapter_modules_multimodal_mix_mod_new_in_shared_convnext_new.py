...
import numpy as np
import math
from .base.twinconvnextshared import TwinConvNeXtShared
def to_3d(x):
...
class SpatialPriorModuleBimodal(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, img_size=(1024,1024),arch='base',checkpoint='https://download.openmmlab.com/mmclassification/v0/convnext/downstream/convnext-base_3rdparty_in21k_20220301-262fd037.pth'):
        ...
        self.twin_conv=TwinConvNeXtShared(
                # arch='tiny',
                arch=arch,
                 in_channels=3,
                 stem_patch_size=4,
    ...
    def forward(self, x,x_other_mod):
        ...
        def _inner_forward(x,x_other_mod):
            ...
            # x, x_other = torch.split(inputs, (3, 3), dim=1)
            # c1,c2,c3,c4 = self.twin_conv(torch.cat((x,x_other_mod),dim=1))
            c1,c2,c3,c4 = self.twin_conv(x,x_other_mod)
            # x_other_mod = self.twin_conv(x_other_mod) 
            # twin_outs = [torch.cat((out_x1, out_y1), dim=1) for out_x1, out_y1 in zip(x,x)]
            # c1,c2,c3,c4=self.twin_conv(torch.cat((x,x_other_mod),dim=1))
