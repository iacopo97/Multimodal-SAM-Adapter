...
from .adapter_modules_multimodal_4mod_selector_image_based_one_adapter_branch_SCORE import SpatialPriorModule, SpatialPriorModulewithGammaSpatial, InteractionBlock, deform_inputs, ModalityScore
...
class SAMAdapterbimodalconcatSCORESMART(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        self.merger=nn.Sequential(*[nn.ReLU(inplace=True),nn.Linear(embed_dim*2,embed_dim)])
        ...
        self.merger.apply(self._init_weights)
    ...
    def forward(self, x):
        ...
        if self.num_mod>1:
            ...
            for ind in range(score.size(2)):
                ...
                c_tot_list.append(score[:,:,ind].unsqueeze(-1)*c_tot[:,:,(ind)*self.embed_dim:(ind+1)*self.embed_dim])
            ...
            c_tot_scored=torch.cat(c_tot_list, dim=2)
            c_tot_merged=self.merger(c_tot_scored)
            c=c_tot_merged[:,:c1.size(1),:]
            c1=c_tot_merged[:,c1.size(1):,:]
