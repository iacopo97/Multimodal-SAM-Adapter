...
from .adapter_modules_multimodal_2_adapter_branch_SCORE_common import SpatialPriorModule, ModalityScore ,InteractionBlock, SpatialPriorModuleGeneric, deform_inputs
...
class SAMAdapterbimodal2branchconcatDEEPER(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
                 ...
                 use_extra_extractor=True, with_cp=True, drop_path_rate=0.4,drop_rate=0., drop_multimodal_path=0.2,bs=2, *args, **kwargs):
        ...
        # self.gamma_init_values=gamma_init_values  and self.num_mod>1
        if ('rgb' in self.modalities_name):
            ...
            self.spm = SpatialPriorModuleGeneric(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=self.in_ch_im)
        ...
        if self.num_mod>1:
            self.spm_other_modalities = SpatialPriorModuleGeneric(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=self.modalities_ch[1])
        ...
        self.interactions = nn.Sequential(*[
            InteractionBlock(dim=embed_dim, num_heads=deform_num_heads, n_points=n_points,
                             init_values=init_values, gamma_init_values=gamma_init_values,drop_path=self.drop_path_rate,
                             norm_layer=self.norm_layer, with_cffn=with_cffn,
                             cffn_ratio=cffn_ratio, deform_ratio=deform_ratio,
                             num_mod=self.num_mod, modalities_name=self.modalities_name, drop_multimodal_path=drop_multimodal_path,
        ...
        self.score=ModalityScore(embed_dim=embed_dim, with_cp=False, num_mod=self.num_mod, inter_ch=32)
        # self.merger1.apply(self._init_weights)
        # self.merger2.apply(self._init_weights)
        self.score.apply(self.init_weights)
        # self.merger1 =nn.Sequential(*[nn.Linear(2*embed_dim, embed_dim/2),nn.GELU(),nn.Linear(embed_dim/2, embed_dim)])
        # self.merger2 =nn.Sequential(*[nn.Linear(2*embed_dim, embed_dim/2),nn.GELU(),nn.Linear(embed_dim/2, embed_dim)])
        # self.merger1.apply(self._init_weights)
        # self.merger2.apply(self._init_weights)
    ...
    def forward(self, x):
        ...
        x=x[:,:self.in_ch_im]
        if ('rgb' in self.modalities_name):
            c1, c2, c3, c4,c1_shape,c2_shape,c3_shape,c4_shape = self.spm(x)
        ...
        # mod_selector_gamma_ext=torch.clone(mod_selector_gamma)
        if self.num_mod>1:
            c1_other_modalities, c2_other_modalities, c3_other_modalities, c4_other_modalities,_,_,_,_ = self.spm_other_modalities(x_other_modalities)
        ...
        for i, layer in enumerate(self.interactions):
            ...
            if self.num_mod>1:
                x,c,c2_4_other_modalities=layer(x, c, self.blocks[indexes[0]:indexes[-1] + 1],
                        deform_inputs1, deform_inputs2, H, W, c2_4_other_modalities)
            elif ('rgb' in self.modalities_name) and (self.num_mod==1):
                x,c,_=layer(x, c, self.blocks[indexes[0]:indexes[-1] + 1],
                            deform_inputs1, deform_inputs2, H, W, None)
        ...
        if self.num_mod>1:
            ...
            c_tot=torch.cat([c1_merged,c_merged],dim=1)
            # c_tot_list=[c1_merged,c_merged]
            # c5_tot=torch.cat([c5,c5_other_modalities],dim=2)
            
            score=self.score(c_tot,c1_shape,c2_shape,c3_shape,c4_shape)
            c_list=[]
            c1_list=[]
            for ind in range(score.size(2)):
                # c_tot[:,:,(ind)*self.embed_dim:(ind+1)*self.embed_dim]=score[:,:,ind].unsqueeze(-1)*c_tot[:,:,(ind)*self.embed_dim:(ind+1)*self.embed_dim]
                c_list.append(score[:,c1.size(1):,ind].unsqueeze(-1)*c_merged[:,:,(ind)*self.embed_dim:(ind+1)*self.embed_dim])
                c1_list.append(score[:,:c1.size(1),ind].unsqueeze(-1)*c1_merged[:,:,(ind)*self.embed_dim:(ind+1)*self.embed_dim])
            # c=self.merger2(c_merged)
            # c1=self.merger1(c1_merged)
            c=torch.sum(torch.stack(c_list,dim=-1),dim=-1)
            c1=torch.sum(torch.stack(c1_list,dim=-1),dim=-1)
            mod_selector_gamma1=score[:,:c1.size(1),:]
            mod_selector_gamma2=score[:,c1.size(1):(c1.size(1)+c2.size(1)),:]
            mod_selector_gamma3=score[:,(c1.size(1)+c2.size(1)):(c1.size(1)+c2.size(1)+c3.size(1)),:]
            mod_selector_gamma4=score[:,(c1.size(1)+c2.size(1)+c3.size(1)):,:]
