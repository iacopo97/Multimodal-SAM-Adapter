...
class SAMAdaptermultimodal4modselectoronebranchModsinglechannelByModDropathnoRGBbranchvarconcat(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        self.modality_merger_1 = nn.Sequential(*[nn.Linear(self.num_mod, self.num_mod-1,bias=True),
                                               nn.ReLU(inplace=True),])
        self.modality_merger_2 = nn.Sequential(*[nn.Linear(self.num_mod, self.num_mod-1,bias=True),
                                               nn.ReLU(inplace=True),])
        self.modality_merger_3 = nn.Sequential(*[nn.Linear(self.num_mod, self.num_mod-1,bias=True),
                                               nn.ReLU(inplace=True),])
        self.modality_merger_4 = nn.Sequential(*[nn.Linear(self.num_mod, self.num_mod-1,bias=True),
                                               nn.ReLU(inplace=True),])
    ...
    def forward(self, x):
        ...
        mod_selector_gamma1=self.modality_merger_1(mod_selector_gamma1)
        mod_selector_gamma2=self.modality_merger_2(mod_selector_gamma2)
        mod_selector_gamma3=self.modality_merger_3(mod_selector_gamma3)
        mod_selector_gamma4=self.modality_merger_4(mod_selector_gamma4)
        ...
        if self.num_mod>1:
            for i in range(1,len(self.modalities_name)):
                c1_other_modalities_t, c2_other_modalities_t, c3_other_modalities_t, c4_other_modalities_t =  mod_selector_gamma1[:,:,i-1].unsqueeze(-1)*c1_l[i-1], mod_selector_gamma2[:,:,i-1].unsqueeze(-1)*c2_l[i-1], mod_selector_gamma3[:,:,i-1].unsqueeze(-1)*c3_l[i-1], mod_selector_gamma4[:,:,i-1].unsqueeze(-1)*c4_l[i-1]
        ...
        # mod_selector_gamma3=torch.stack(mod_selector_gamma3_l,dim=-1)
        # mod_selector_gamma4=torch.stack(mod_selector_gamma4_l,dim=-1)
        mod_selector_gamma1=mod_selector_gamma1.transpose(1, 2).view(bs, self.num_mod-1, 4*H, 4*W).contiguous() #H and W are H/16 and W/16
        mod_selector_gamma2 = mod_selector_gamma2.transpose(1, 2).view(bs, self.num_mod-1, H * 2, W * 2).contiguous()
        mod_selector_gamma3 = mod_selector_gamma3.transpose(1, 2).view(bs, self.num_mod-1, H, W).contiguous()
        mod_selector_gamma4 = mod_selector_gamma4.transpose(1, 2).view(bs, self.num_mod-1, H // 2, W // 2).contiguous()
