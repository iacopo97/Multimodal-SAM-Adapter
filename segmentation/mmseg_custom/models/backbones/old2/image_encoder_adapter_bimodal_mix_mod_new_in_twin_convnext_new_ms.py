...
from .adapter_modules_multimodal_mix_mod_new_in_twin_convnext_new_ms import SpatialPriorModule, SpatialPriorModuleBimodal, InteractionBlock, deform_inputs
...
class SAMAdapterbimodalMixModNewInTwinCONVNEWMS(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        self.mod_sel_1=nn.Sequential(nn.Linear(embed_dim*2,1))#,nn.Sigmoid()
        self.mod_sel_1.apply(self._init_weights)
        self.up_1 = nn.ConvTranspose2d(1, 1, 2, 2)
        self.up_2 = nn.ConvTranspose2d(1, 1, 2, 2)
        self.down_4 = nn.Conv2d(1, 1, kernel_size=3, stride=2, padding=1, bias=False)
        self.up_1.apply(self._init_weights)
        self.up_2.apply(self._init_weights)
        self.down_4.apply(self._init_weights)
        self.mod_sel_2=nn.Sequential(nn.Linear(embed_dim*2,1))#,nn.Sigmoid()
        self.sigm=nn.Sigmoid()
        self.mod_sel_2.apply(self._init_weights)
        self.up_1_other_mod = nn.ConvTranspose2d(1, 1, 2, 2)
        self.up_2_other_mod = nn.ConvTranspose2d(1, 1, 2, 2)
        self.down_4_other_mod = nn.Conv2d(1, 1,  kernel_size=3, stride=2, padding=1, bias=False)
        self.up_1_other_mod.apply(self._init_weights)
        self.up_2_other_mod.apply(self._init_weights)
        self.down_4_other_mod.apply(self._init_weights)
        # if self.num_mod>1:
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            ...
            # else:
            c1, c2, c3, c4,c1_other,c2_other,c3_other,c4_other = self.spm(x,x_other_modalities)
            # c2, c3, c4 = self._add_level_embed(c2, c3, c4)
            # c2_other,c3_other,c4_other = self._add_level_embed(c2_other, c3_other, c4_other)  
            # c = torch.cat([c2, c3, c4], dim=1)
            # c_other=torch.cat([c2_other, c3_other, c4_other], dim=1)
        ...
        # x=x+pos_embed
        x3=x.transpose(1,2).view(bs, dim, H, W)
        sel1=torch.cat([c3, x3], dim=1)
        sel1=sel1.view(bs,2*dim,-1).transpose(1,2)
        mod_1=self.mod_sel_1(sel1)
        mod_1=mod_1.transpose(1,2).view(bs,1,H,W)
        sel2=torch.cat([c3_other, x3], dim=1)
        sel2=sel2.view(bs,2*dim,-1).transpose(1,2)
        mod_2=self.mod_sel_2(sel2)
        mod_2=mod_2.transpose(1,2).view(bs,1,H,W)
        
        
        mod_12=self.up_2(mod_1)
        mod_11=self.up_1(mod_12)
        mod_14=self.down_4(mod_1)
        mod_11=self.sigm(mod_11)
        mod_12=self.sigm(mod_12)
        mod_13=self.sigm(mod_1)
        mod_14=self.sigm(mod_14)
        ...
        mod_22=self.up_2_other_mod(mod_2)
        mod_21=self.up_1_other_mod(mod_22)
        mod_24=self.down_4_other_mod(mod_2)
        mod_21=self.sigm(mod_21)
        mod_22=self.sigm(mod_22)
        mod_23=self.sigm(mod_2)
        mod_24=self.sigm(mod_24)
        
        c1_other=c1_other*mod_21
        c2_other=c2_other*mod_22
        c3_other=c3_other*mod_23
        c4_other=c4_other*mod_24
