...
from .adapter_modules_multimodal_gamma_spatial_image_based_no_gamma_extractor_no_gammac1 import SpatialPriorModule, SpatialPriorModulewithGammaSpatial, InteractionBlock, deform_inputs
...
class SAMAdaptermultimodalselectorgamma_nogammac1(ImageEncoderViT):
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            # SPM forward
            # c1, c2, c3, c4, mod_selector_gamma2, mod_selector_gamma3, mod_selector_gamma4, mod_selector_gamma_inj = self.spm_gamma_spatial(x)
            c1, c2, c3, c4, mod_selector_gamma_inj = self.spm_gamma_spatial(x)
        ...
        for i, layer in enumerate(self.interactions):
            ...
            if self.num_mod>1:
                x,c,c2_4_other_modalities=layer(x, c, self.blocks[indexes[0]:indexes[-1] + 1],
                        deform_inputs1, deform_inputs2, H, W, c2_4_other_modalities, mod_selector_gamma_inj)
            elif ('rgb' in self.modalities_name) and (self.num_mod==1):
                x,c,_=layer(x, c, self.blocks[indexes[0]:indexes[-1] + 1],
                            deform_inputs1, deform_inputs2, H, W, None, mod_selector_gamma_inj)
        ...
        if ('rgb' in self.modalities_name) and (self.num_mod==1):
            ...
            for i in range(1,len(self.modalities_name)):
                ...
                # c2_4_other_modalities_cumulative+=self.multimodal_drop_path(mod_selector_gamma[:,:,i].unsqueeze(-1)*c2_4_other_modalities[i-1])
                c2_4_other_modalities_cumulative+=self.multimodal_drop_path(c2_4_other_modalities[i-1]) #mod_selector_gamma[:,:,i].unsqueeze(-1)*
                c1_other_modalities_cumulative+=self.multimodal_drop_path(c1_other_modalities[i-1])
            # c=mod_selector_gamma[:,:,0].unsqueeze(-1)*c+c2_4_other_modalities_cumulative
        ...
        return [f1, f2, f3, f4], mod_selector_gamma_inj
