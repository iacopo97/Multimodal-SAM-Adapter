...
from .adapter_modules_multimodal_4mod_selector_image_based_one_adapter_branch_modality_selector_from_single_channel_by_mod_lighter_sam_modGELU import SpatialPriorModule, SpatialPriorModuleModSelectorsinglechannel, InteractionBlock, ModalitySelectorViT, deform_inputs
...
class SAMAdaptermultimodal4modselectoronebranchModsinglechannelByModlighterSAMmodGELU(ImageEncoderViT):
    ...
    def forward(self, x):
        ...
        x=x[:,:self.in_ch_im]
        mod_selector_gamma1_l=list()
        mod_selector_gamma2_l=list()
        mod_selector_gamma3_l=list()
        mod_selector_gamma4_l=list()
        c1_l=list()
        c2_l=list()
        c3_l=list()
        c4_l=list()
        ...
        if self.num_mod>1:
            # SPM forward
            for i in range(1,len(self.modalities_name)):
                mod_selector_gamma1_t,mod_selector_gamma2_t,mod_selector_gamma3_t, mod_selector_gamma4_t,c1_other_modalities, c2_other_modalities, c3_other_modalities, c4_other_modalities = self.spm_other_modalities[f"spm_{self.modalities_name[i]}"](x_other_modalities[:,modalities_ch_cumsum[i-1]:modalities_ch_cumsum[i]])
        ...
        mod_selector_gamma1_sam,mod_selector_gamma2_sam,mod_selector_gamma3_sam,mod_selector_gamma4_sam=self.mod_selector(x,c1_shape_orig, c2_shape_orig,c3_shape_orig,c4_shape_orig)
        mod_selector_gamma1=mod_selector_gamma1*mod_selector_gamma1_sam
        mod_selector_gamma2=mod_selector_gamma2*mod_selector_gamma2_sam
        mod_selector_gamma3=mod_selector_gamma3*mod_selector_gamma3_sam
        mod_selector_gamma4=mod_selector_gamma4*mod_selector_gamma4_sam
        bs, n, dim = x.shape
