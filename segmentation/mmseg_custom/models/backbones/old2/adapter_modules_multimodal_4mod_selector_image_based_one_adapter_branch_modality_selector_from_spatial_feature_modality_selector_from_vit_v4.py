...
class SpatialPriorModulecombinedspatial(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3):
        ...
        self.fc_mod1=nn.Conv2d(inplanes, embed_dim//4, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc_mod2=nn.Conv2d(2*inplanes, embed_dim//4, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc_mod3=nn.Conv2d(4*inplanes, embed_dim//4, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc_mod4=nn.Conv2d(4*inplanes, embed_dim//4, kernel_size=1, stride=1, padding=0, bias=True)
...
class ModalityCombiner(nn.Module):
    def __init__(self, embed_dim, with_cp=False, num_mod=4):
        ...
        self.with_cp = with_cp
        self.num_mod = num_mod
        self.channel_equalizer=nn.Conv2d(embed_dim, embed_dim//4, kernel_size=1, stride=1, padding=0, bias=True)
        # self.merger=nn.Conv2d(embed_dim//2, embed_dim//4, kernel_size=1, stride=1, padding=0, bias=True)
        self.conv1 = nn.Sequential(*[
            nn.Conv2d(5*embed_dim//4, num_mod, kernel_size=1, stride=1, padding=0, bias=True),
            nn.ReLU(inplace=True) #add Sigmoid
        ])
