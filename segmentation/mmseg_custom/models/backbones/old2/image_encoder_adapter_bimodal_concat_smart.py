...
class SAMAdapterbimodalconcatSMART(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        # self.score=ModalityScore(embed_dim,2)
    ...
    def forward(self, x):
        ...
        if self.num_mod>1:
            ...
            # score=self.score(c_merged,c1_merged)
            c_tot_merged=torch.cat([c1_merged,c_merged],dim=1)
            c_tot=self.merger(c_tot_merged)
            c1=c_tot[:, 0:c1.size(1), :]
            c=c_tot[:, c1.size(1):, :]
