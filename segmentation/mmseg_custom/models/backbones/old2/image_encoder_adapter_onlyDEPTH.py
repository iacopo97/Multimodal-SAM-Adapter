...
from .adapter_modules_rgbdepth import SpatialPriorModule, InteractionBlock, deform_inputs
...
class SAMAdapterRGBDEPTH(ImageEncoderViT):
    ...
    def forward(self, x):
        ...
        x_depth=x[:,self.in_ch_im:]
        x_rgb=x[:,:self.in_ch_im]
        # if ('rgb' in self.modalities_name):
            # SPM forward
        c1, c2, c3, c4 = self.spm(x_depth)
        c2, c3, c4 = self._add_level_embed(c2, c3, c4)
        c = torch.cat([c2, c3, c4], dim=1)
        
        deform_inputs1, deform_inputs2 = deform_inputs(x_depth)
        # if self.num_mod>1:
