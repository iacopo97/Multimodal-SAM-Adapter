...
from .adapter_modules_multimodal_4mod_selector_image_based_one_adapter_branch_SCORE_common import SpatialPriorModule, SpatialPriorModuleGeneric, InteractionBlock, deform_inputs, ModalityScore #SpatialPriorModulewithGammaSpatial
...
class SAMAdapterbimodalSCOREcommonLORA(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        # self.merger1=nn.Sequential(*[nn.ReLU(inplace=True),nn.Linear(embed_dim*2,embed_dim)])
        # self.merger2=nn.Sequential(*[nn.ReLU(inplace=True),nn.Linear(embed_dim*2,embed_dim)])
    ...
    def forward(self, x):
        ...
        if self.num_mod>1:
            ...
            # c1_merged_scored=torch.cat(c1_list,dim=2)
            # c_merged_scored=torch.cat(c_list,dim=2)
            # c_tot_merged=torch.cat(c_list,dim=2)
            # c1_merged_scored=c_tot_merged[:,0:c1.size(1),:]
            # c_merged_scored=c_tot_merged[:,c1.size(1):,:]
            # c=self.merger1(c_merged_scored)
            # c1=self.merger2(c1_merged_scored)
