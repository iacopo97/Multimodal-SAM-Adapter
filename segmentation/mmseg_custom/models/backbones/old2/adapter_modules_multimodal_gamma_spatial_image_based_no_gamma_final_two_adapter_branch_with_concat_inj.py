...
class Extractor_2(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1, deform_ratio=1.0,
        ...
        self.attn_other_mod = MSDeformAttn(d_model=dim, n_levels=n_levels, n_heads=num_heads,
                                 n_points=n_points, ratio=deform_ratio)
        self.with_cffn = with_cffn
        self.with_cp = with_cp
        if with_cffn:
            self.ffn = ConvFFN(in_features=dim, hidden_features=int(dim * cffn_ratio), drop=drop)
            self.ffn_other_mod = ConvFFN(in_features=dim, hidden_features=int(dim * cffn_ratio), drop=drop)
            self.ffn_norm = norm_layer(dim)
            self.ffn_norm_other_mod = norm_layer(dim)
            self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()

    def forward(self, query, query_other_mod, reference_points, feat, spatial_shapes, level_start_index, H, W):
        
        def _inner_forward(query, query_other_mod, feat):

            attn = self.attn(self.query_norm(query), reference_points,
                             self.feat_norm(feat), spatial_shapes,
                             level_start_index, None)
            attn_other_mod = self.attn_other_mod(self.query_norm_other_mod(query_other_mod), reference_points,
                             self.feat_norm(feat), spatial_shapes,
                             level_start_index, None)
            query = query + attn + attn_other_mod
            query_other_mod = query_other_mod + attn_other_mod + attn
    
            if self.with_cffn:
                query = query + self.drop_path(self.ffn(self.ffn_norm(query), H, W))
                query_other_mod = query_other_mod + self.drop_path(self.ffn_other_mod(self.ffn_norm_other_mod(query_other_mod), H, W))
            return query, query_other_mod
        ...
        if self.with_cp and query.requires_grad:
            ...
            query, query_other_mod = cp.checkpoint(_inner_forward, query, query_other_mod, feat)
            # print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=10))
        else:
            query, query_other_mod = _inner_forward(query, query_other_mod, feat)
...
class Injector_2(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, n_levels=1, deform_ratio=1.0,
        ...
        self.query_norm = norm_layer(dim)
        self.feat_norm = norm_layer(dim)
        self.feat_norm_other_mod = norm_layer(dim)
        self.attn = MSDeformAttn(d_model=dim, n_levels=n_levels, n_heads=num_heads,
                                 n_points=n_points, ratio=deform_ratio)
        self.attn_other_mod=MSDeformAttn(d_model=dim, n_levels=n_levels, n_heads=num_heads,n_points=n_points, ratio=deform_ratio)
        self.gamma = nn.Parameter(init_values * torch.ones((dim)), requires_grad=True)
        self.merge=nn.Linear(2*dim,dim)
        # self.gamma_other_mod = nn.Parameter(init_values * torch.ones((dim)), requires_grad=True)

    def forward(self, query, reference_points, feat, feat_other_mod, spatial_shapes, level_start_index):
        
        def _inner_forward(query, feat, feat_other_mod):

            attn_1 = self.attn(self.query_norm(query), reference_points,
                             self.feat_norm(feat), spatial_shapes,
                             level_start_index, None)
            if feat_other_mod is None:
                return query + self.gamma * attn_1
            else:
                attn_2 = self.attn_other_mod(self.query_norm(query), reference_points,
                                            self.feat_norm_other_mod(feat_other_mod), spatial_shapes,
                                            level_start_index, None)
                attn_all=torch.cat((attn_1,attn_2),2)
                attn_all_merge=self.merge(attn_all)
                return query + self.gamma * attn_all_merge
            
        if self.with_cp and query.requires_grad:
            # with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes=True, profile_memory=True, use_cuda=True) as prof:
                # with record_function("model_inference"):
            query = cp.checkpoint(_inner_forward, query, feat, feat_other_mod)
            # print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=10))
        else:
            query = _inner_forward(query, feat, feat_other_mod)
...
class InteractionBlock(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        ...
        self.injector = Injector_2(dim=dim, n_levels=3, num_heads=num_heads, init_values=init_values,
                                 n_points=n_points, norm_layer=norm_layer, deform_ratio=deform_ratio,
                                 with_cp=False)
        self.extractor = Extractor_2(dim=dim, n_levels=1, num_heads=num_heads, n_points=n_points,
                                   norm_layer=norm_layer, deform_ratio=deform_ratio, with_cffn=with_cffn,
                                   cffn_ratio=cffn_ratio, drop=drop, drop_path=drop_path, with_cp=False)
        if extra_extractor:
            self.extra_extractors = nn.Sequential(*[
                Extractor_2(dim=dim, num_heads=num_heads, n_points=n_points, norm_layer=norm_layer,
    ...
    def forward(self, x, c, c_other_mod, blocks, deform_inputs1, deform_inputs2, H, W):
        x = self.injector(query=x, reference_points=deform_inputs1[0],
                          feat=c, feat_other_mod=c_other_mod,spatial_shapes=deform_inputs1[1],
                          level_start_index=deform_inputs1[2])
        for idx, blk in enumerate(blocks):
            x = blk(x, H, W)
        c, c_other_mod = self.extractor(query=c, query_other_mod=c_other_mod, reference_points=deform_inputs2[0],
                           feat=x, spatial_shapes=deform_inputs2[1],
                           level_start_index=deform_inputs2[2], H=H, W=W)
        if self.extra_extractors is not None:
            for extractor in self.extra_extractors:
                c, c_other_mod = extractor(query=c, query_other_mod=c_other_mod, reference_points=deform_inputs2[0],
                              feat=x, spatial_shapes=deform_inputs2[1],
                              level_start_index=deform_inputs2[2], H=H, W=W)
        return x, c, c_other_mod
