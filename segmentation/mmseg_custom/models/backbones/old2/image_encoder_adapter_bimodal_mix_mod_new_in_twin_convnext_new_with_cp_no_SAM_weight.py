...
from .base.image_encoder_no_weight import ImageEncoderViTnoweight as ImageEncoderViT
from .adapter_modules_multimodal_mix_mod_new_in_twin_convnext_new_with_cp import SpatialPriorModule, SpatialPriorModuleBimodal, InteractionBlock, deform_inputs
...
class SAMAdapterbimodalMixModNewInTwinConvNEWwithcpnoweight(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
                 ...
                 use_extra_extractor=True, with_cp=True, drop_path_rate=0.4,drop_rate=0.,conv_drop_path_rate=0.4, drop_multimodal_path=0.2, arch='base',checkpoint='check', *args, **kwargs):
        ...
        if ('rgb' in self.modalities_name) and self.num_mod>1:
            ...
            self.spm = SpatialPriorModuleBimodal(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=self.in_ch_im,conv_drop_path_rate=conv_drop_path_rate, img_size=self.img_size,arch=arch,checkpoint=checkpoint)
