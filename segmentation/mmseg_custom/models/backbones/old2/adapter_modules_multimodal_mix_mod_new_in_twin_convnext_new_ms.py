...
class SpatialPriorModuleBimodal(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, img_size=(1024,1024),arch='base',checkpoint='https://download.openmmlab.com/mmclassification/v0/convnext/downstream/convnext-base_3rdparty_in21k_20220301-262fd037.pth'):
        ...
        #     nn.ReLU(inplace=True)
        # ])
        self.fc1 = nn.Conv2d(inplanes*2, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc1.apply(self._init_weights)
        self.fc2 = nn.Conv2d(inplanes*4, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc2.apply(self._init_weights)
        self.fc3 = nn.Conv2d(inplanes*8, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc3.apply(self._init_weights)
        self.fc4 = nn.Conv2d(inplanes*16, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc4.apply(self._init_weights)
        
        self.fc1_other_mod = nn.Conv2d(inplanes*2, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc1_other_mod.apply(self._init_weights)
        self.fc2_other_mod = nn.Conv2d(inplanes*4, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc2_other_mod.apply(self._init_weights)
        self.fc3_other_mod = nn.Conv2d(inplanes*8, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc3_other_mod.apply(self._init_weights)
        self.fc4_other_mod = nn.Conv2d(inplanes*16, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc4_other_mod.apply(self._init_weights)
        ...
        #          norm_cfg=dict(type='GN', num_groups=32))
        # self.smart_fusion = RoadFormer2Neck(in_channels=[2*inplanes,4*inplanes,8*inplanes,inplanes*8],
        #          out_channels=[2*inplanes,4*inplanes,8*inplanes,inplanes*8],
        #          layer = len([2*inplanes,4*inplanes,8*inplanes,inplanes*8]),
        #          img_scale=(img_size, img_size),
        #          norm_cfg=dict(type='GN', num_groups=32))
    ...
    def forward(self, x,x_other_mod):
        ...
        def _inner_forward(x,x_other_mod):
            ...
            # c1,c2,c3,c4=self.smart_fusion([c1_merged,c2_merged,c3_merged,c4_merged])
            # c1,c2,c3,c4=self.smart_fusion([c1,c2,c3,c4])
            c1_other_mod = c1[:,2*self.inplanes:]
            c2_other_mod = c2[:,4*self.inplanes:]
            c3_other_mod = c3[:,8*self.inplanes:]
            c4_other_mod = c4[:,16*self.inplanes:]
            c1 = self.fc1(c1[:,:2*self.inplanes])
            c2 = self.fc2(c2[:,:4*self.inplanes])
            c3 = self.fc3(c3[:,:8*self.inplanes])
            c4 = self.fc4(c4[:,:16*self.inplanes])
            
            c1_other_mod = self.fc1_other_mod(c1_other_mod)
            c2_other_mod = self.fc2_other_mod(c2_other_mod)
            c3_other_mod = self.fc3_other_mod(c3_other_mod)
            c4_other_mod = self.fc4_other_mod(c4_other_mod)
            ...
            # c3 = c3.view(bs, dim, -1).transpose(1, 2)  # 16s
            # c4 = c4.view(bs, dim, -1).transpose(1, 2)  # 32s
            # c1_other_mod= c1_other_mod.view(bs, dim, -1).transpose(1, 2)
            # c2_other_mod= c2_other_mod.view(bs, dim, -1).transpose(1, 2)
            # c3_other_mod= c3_other_mod.view(bs, dim, -1).transpose(1, 2)
            # c4_other_mod= c4_other_mod.view(bs, dim, -1).transpose(1, 2)
    
            return c1, c2, c3, c4, c1_other_mod, c2_other_mod, c3_other_mod, c4_other_mod
