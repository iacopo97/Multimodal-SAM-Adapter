...
from .base.image_encoder import PatchEmbed
from .adapter_modules_multimodal_2_adapter_branch_sequential_ADA import SpatialPriorModule, InteractionBlock, deform_inputs
...
class SAMAdapterbimodal2branchseqADA(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        if self.num_mod>1:
            self.patch_embed_modality = PatchEmbed(kernel_size=(16, 16),
            stride=(16, 16),
            in_chans=self.modalities_ch[1],
            embed_dim=embed_dim,
            )
    ...
    def _add_level_embed(self, c2, c3, c4,select_mod=None):
        # global COUNTER
    ...
    def forward(self, x):
        ...
        # mod_selector_gamma_ext=torch.clone(mod_selector_gamma)
        if self.num_mod>1:
            x_other_modalities,H,W=self.patch_embed_modality(x_other_modalities)
            pos_embed=self._get_pos_embed1(self.pos_embed, H, W)
            #pos_embed=pos_embed.flatten(1,2)
            x_other_modalities = self.pos_drop(x_other_modalities + pos_embed)
        ...
        for i, layer in enumerate(self.interactions):
            ...
            if self.num_mod>1:
                x,c,x_other_modalities=layer(x, c, self.blocks[indexes[0]:indexes[-1] + 1],
                        deform_inputs1, deform_inputs2, H, W, x_other_modalities)
            elif ('rgb' in self.modalities_name) and (self.num_mod==1):
