...
from .adapter_modules_multimodal_4mod_selector_image_based_one_adapter_branch_modality_selector_from_spatial_feature_modality_selector_from_vit_v4 import SpatialPriorModule, ModalityCombiner, SpatialPriorModulecombinedspatial, InteractionBlock, deform_inputs
...
class SAMAdapterbimodal4modselectoronebranchModFromSpModfromViTv4(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        if ('rgb' in self.modalities_name):
            ...
            self.spm_comb_spatial = SpatialPriorModulecombinedspatial(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=self.in_ch_im)
            self.up = nn.ConvTranspose2d(embed_dim, embed_dim, 2, 2)
            # self.mod_selector=ModalitySelectorViT(embed_dim=embed_dim, with_cp=False, num_mod=self.num_mod)
            self.mod_combiner = ModalityCombiner(embed_dim=embed_dim, with_cp=False, num_mod=self.num_mod)
            ##INITIALIZE THEM###
            self.up.apply(self._init_weights)
            self.spm_comb_spatial.apply(self._init_weights)
            # self.mod_selector.apply(self._init_weights)
            self.mod_combiner.apply(self._init_weights)
            ###################       
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            ...
            # c1, c2, c3, c4, mod_selector_gamma1, mod_selector_gamma2, mod_selector_gamma3, mod_selector_gamma4 = self.spm_gamma_spatial(x)
            mod_t1,mod_t2,mod_t3,mod_t4,c1,c2,c3,c4=self.spm_comb_spatial(x) #mod_selector_gamma2_t,mod_selector_gamma3_t,mod_selector_gamma4_t
            # mod_selector_gamma1_t=(mod_selector_gamma1_t)/torch.max(mod_selector_gamma1_t.sum(dim=-1,keepdim=True), 1e-12*torch.ones_like(mod_selector_gamma1.sum(dim=-1,keepdim=True)))
            # mod_selector_gamma2_t=(mod_selector_gamma2_t)/torch.max(mod_selector_gamma2_t.sum(dim=-1,keepdim=True), 1e-12*torch.ones_like(mod_selector_gamma2.sum(dim=-1,keepdim=True)))
            # mod_selector_gamma3_t=(mod_selector_gamma3_t)/torch.max(mod_selector_gamma3_t.sum(dim=-1,keepdim=True), 1e-12*torch.ones_like(mod_selector_gamma3.sum(dim=-1,keepdim=True)))
            # mod_selector_gamma4_t=(mod_selector_gamma4_t)/torch.max(mod_selector_gamma4_t.sum(dim=-1,keepdim=True), 1e-12*torch.ones_like(mod_selector_gamma4.sum(dim=-1,keepdim=True)))
        ...
        c1_shape_orig=c1.transpose(1, 2).view(bs, dim, 4*H, 4*W).shape
        c2_shape_orig=c2.transpose(1, 2).view(bs, dim, H * 2, W * 2).shape
        c3_shape_orig=c3.transpose(1, 2).view(bs, dim, H, W).shape
        c4_shape_orig=c4.transpose(1, 2).view(bs, dim, H // 2, W // 2).shape
        # mod_selector_gamma3_vit=self.mod_selector(x,c3_shape_orig)
        mod_selector_gamma1,mod_selector_gamma2,mod_selector_gamma3,mod_selector_gamma4=self.mod_combiner(mod_t1,mod_t2,mod_t3,mod_t4, x, c1_shape_orig, c2_shape_orig, c3_shape_orig, c4_shape_orig)
        # mod_selector_gamma1=torch.cat([mod_selector_gamma1_t,F.interpolate(mod_selector_gamma3_vit, size=c1_shape_orig[-2:], mode='bilinear', align_corners=False)],dim=1)
        # mod_selector_gamma1=mod_selector_gamma1_t*mod_selector_gamma1_vit
        # mod_selector_gamma2=mod_selector_gamma2_t*mod_selector_gamma2_vit
        # mod_selector_gamma3=mod_selector_gamma3_t*mod_selector_gamma3_vit
        # mod_selector_gamma4=mod_selector_gamma4_t*mod_selector_gamma4_vit
