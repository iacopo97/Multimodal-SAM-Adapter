...
from .adapter_modules_multimodal_4mod_selector_image_based_one_adapter_branch_modality_selector_from_spatial_feature_SOFTMAX import SpatialPriorModule, SpatialPriorModuleModSelector, InteractionBlock, deform_inputs
...
class SAMAdaptermultimodal4modselectoronebranchModFromSpSOFTMAXDROPATH(ImageEncoderViT):
    ...
    def forward(self, x):
        ...
        if self.num_mod>1:
            # SPM forward
            for i in range(1,len(self.modalities_name)):
                c1_other_modalities, c2_other_modalities, c3_other_modalities, c4_other_modalities = self.multimodal_drop_path(self.spm_other_modalities[f"spm_{self.modalities_name[i]}"](x_other_modalities[:,modalities_ch_cumsum[i-1]:modalities_ch_cumsum[i]]))
