...
class SAMAdaptermultimodal4modselectoronebranchRelunormlevemb(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        if self.num_mod>1:
            self.level_embed_mod = nn.Parameter(torch.zeros(4, self.num_mod))
    ...
    def _add_level_embed_mod(self, mod1,mod2,mod3,mod4):
        # global COUNTER
        mod1 = mod1 + self.level_embed_mod[0].type_as(mod1)
        # COUNTER+=1
        mod2 = mod2 + self.level_embed_mod[1].type_as(mod2)
        # COUNTER+=1
        mod3 = mod3 + self.level_embed_mod[2].type_as(mod3)
        mod4 = mod4 + self.level_embed_mod[3].type_as(mod4)
        # COUNTER+=1
        # if COUNTER==3*self.num_mod:
        #     COUNTER=0
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            ...
            mod_selector_gamma1,mod_selector_gamma2,mod_selector_gamma3,mod_selector_gamma4=self._add_level_embed_mod(mod_selector_gamma1,mod_selector_gamma2,mod_selector_gamma3,mod_selector_gamma4)
