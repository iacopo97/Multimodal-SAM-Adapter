...
from .adapter_modules_multimodal_gamma_spatial_image_based import SpatialPriorModule, SpatialPriorModulewithGammaSpatial, InteractionBlock, deform_inputs
...
class SAMAdaptermultimodalselectorgamma(ImageEncoderViT):
    ...
    def forward(self, x):
        ...
        mod_selector_gamma_inj=torch.clone(mod_selector_gamma3)
        ...
        for i, layer in enumerate(self.interactions):
            ...
            if self.num_mod>1:
                x,c,c2_4_other_modalities=layer(x, c, self.blocks[indexes[0]:indexes[-1] + 1],
                        deform_inputs1, deform_inputs2, H, W, c2_4_other_modalities, mod_selector_gamma_inj, mod_selector_gamma_ext)
            elif ('rgb' in self.modalities_name) and (self.num_mod==1):
                x,c,_=layer(x, c, self.blocks[indexes[0]:indexes[-1] + 1],
                            deform_inputs1, deform_inputs2, H, W, None, mod_selector_gamma_inj, mod_selector_gamma_ext)
