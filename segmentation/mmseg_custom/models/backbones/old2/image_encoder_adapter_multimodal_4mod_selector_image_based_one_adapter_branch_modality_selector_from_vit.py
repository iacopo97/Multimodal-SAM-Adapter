...
from .adapter_modules_multimodal_4mod_selector_image_based_one_adapter_branch_modality_selector_from_vit import SpatialPriorModule, ModalitySelectorViT, InteractionBlock, deform_inputs
...
class SAMAdaptermultimodal4modselectoronebranchModFromViT(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        if ('rgb' in self.modalities_name):
            ...
            # self.spm_mod_selector = SpatialPriorModuleModSelector(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=self.in_ch_im, num_mod=self.num_mod)
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            ...
            # mod_selector_gamma1,mod_selector_gamma2,mod_selector_gamma3,mod_selector_gamma4,c1,c2,c3,c4=self.spm_mod_selector(x)
            # mod_selector_gamma1=(mod_selector_gamma1)/mod_selector_gamma1.sum(dim=-1,keepdim=True)
        ...
        mod_selector_gamma1,mod_selector_gamma2,mod_selector_gamma3,mod_selector_gamma4=self.mod_selector(x,c1_shape_orig, c2_shape_orig,c3_shape_orig,c4_shape_orig)
        ...
        if self.num_mod>1:
            for i in range(1,len(self.modalities_name)):
                c1_other_modalities, c2_other_modalities, c3_other_modalities, c4_other_modalities = self.spm_other_modalities[f"spm_{self.modalities_name[i]}"](x_other_modalities[:,modalities_ch_cumsum[i-1]:modalities_ch_cumsum[i]])
                c1_other_modalities, c2_other_modalities, c3_other_modalities, c4_other_modalities =  mod_selector_gamma1[:,:,i].unsqueeze(-1)*c1_other_modalities, mod_selector_gamma2[:,:,i].unsqueeze(-1)*c2_other_modalities, mod_selector_gamma3[:,:,i].unsqueeze(-1)*c3_other_modalities, mod_selector_gamma4[:,:,i].unsqueeze(-1)*c4_other_modalities
                c2_other_modalities, c3_other_modalities, c4_other_modalities = self._add_level_embed(c2_other_modalities, c3_other_modalities, c4_other_modalities)
                c2_4_other_modalities = torch.cat([c2_other_modalities, c3_other_modalities, c4_other_modalities], dim=1)
                c=c+c2_4_other_modalities
                c1=c1+c1_other_modalities
