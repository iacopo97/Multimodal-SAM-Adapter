...
from .adapter_modules_multimodal_mix_mod_new_in_twin_convnext import SpatialPriorModule, SpatialPriorModuleBimodal, InteractionBlock, deform_inputs
...
class SAMAdapterbimodalMixModNewInTwinConv(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        if ('rgb' in self.modalities_name) and self.num_mod>1:
            ...
            self.spm = SpatialPriorModuleBimodal(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=self.in_ch_im, img_size=self.img_size)
