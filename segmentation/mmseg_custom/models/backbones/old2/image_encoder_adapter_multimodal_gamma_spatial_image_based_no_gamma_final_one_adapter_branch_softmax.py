...
class SAMAdaptermultimodalselectorgamma_nogammafinalonebranchSoftmax(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        self.softmax_mod1=nn.Softmax(dim=2)
        self.softmax_mod2=nn.Softmax(dim=2)
        self.softmax_mod3=nn.Softmax(dim=2)
        self.softmax_mod4=nn.Softmax(dim=2)
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            ...
            mod_selector_gamma1=self.softmax_mod1(mod_selector_gamma1)
            mod_selector_gamma2=self.softmax_mod2(mod_selector_gamma2)
            mod_selector_gamma3=self.softmax_mod3(mod_selector_gamma3)
            mod_selector_gamma4=self.softmax_mod4(mod_selector_gamma4)
