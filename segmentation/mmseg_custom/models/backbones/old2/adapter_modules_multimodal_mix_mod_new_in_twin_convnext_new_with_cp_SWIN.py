...
from .base.twin_swin import TWINSwinTransformer
def to_3d(x):
...
class SpatialPriorModuleBimodal(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3,conv_drop_path_rate=0.4, img_size=(1024,1024),arch='base',checkpoint='https://download.openmmlab.com/mmclassification/v0/convnext/downstream/convnext-base_3rdparty_in21k_20220301-262fd037.pth'):
        super().__init__()
        self.with_cp = with_cp
        # inplanes=64
        # inplanes=48
        ...
        # print(conv_drop_path_rate)
        self.twin_conv=TWINSwinTransformer(arch=arch,img_size=img_size,drop_path_rate=0.3,
        ...
        #          linear_pw_conv=True,
        #          use_grn=False,
        #          drop_path_rate=conv_drop_path_rate,#0.5?
        ...
        self.smart_fusion = RoadFormer2Neck(in_channels=[4*inplanes,8*inplanes,16*inplanes,inplanes*32],
                 ...
                 img_scale=(img_size, img_size),
                 norm_cfg=dict(type='GN', num_groups=32),with_cp=True)
