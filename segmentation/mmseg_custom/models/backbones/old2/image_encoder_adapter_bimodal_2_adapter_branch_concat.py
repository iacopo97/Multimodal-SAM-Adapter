...
from .adapter_modules_multimodal_2_adapter_branch import SpatialPriorModule, InteractionBlock, deform_inputs
...
def save_map(tensor, name):
    if not os.path.exists('/media/data4/sora/projects/Multimodal/vit-adapter-mm/segmentation/DEBUG/debug_maps'):
        os.makedirs('/media/data4/sora/projects/Multimodal/vit-adapter-mm/segmentation/DEBUGdebug_maps')
    file_path = os.path.join('/media/data4/sora/projects/Multimodal/vit-adapter-mm/segmentation/DEBUGdebug_maps', f'{name}.png')
    tensor = tensor.detach().cpu()
    if tensor.dim() == 4:  # If the tensor is a batch of images, take the first one
        tensor = tensor[0]
    if tensor.size(0) == 1:  # If the tensor is single-channel, convert to 3-channel by repeating
        tensor = tensor.repeat(3, 1, 1)
    elif tensor.size(0) == 2:  # If the tensor is two-channel, add a third channel of zeros
        zeros = torch.zeros(1, tensor.size(1), tensor.size(2))
        tensor = torch.cat((tensor, zeros), dim=0)
    tensor = (tensor - tensor.min()) / (tensor.max() - tensor.min())  # Normalize to [0, 1]
    tensor = tensor.permute(1, 2, 0).numpy()  # Convert to HWC format for saving
    import matplotlib.pyplot as plt
    plt.imsave(file_path, tensor)
    _logger.info(f'Saved {name} map to {file_path}')
#  backbone=dict( 
#         _delete_=True,
#         type='SAMAdapter',
...
class SAMAdapterbimodal2branchconcat(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        self.merger1 = nn.Linear(2*embed_dim, embed_dim)
        self.merger2 = nn.Linear(2*embed_dim, embed_dim)
