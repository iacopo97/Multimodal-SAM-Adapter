...
class SpatialPriorModuleBimodal(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, img_size=(1024,1024),arch='base',checkpoint='https://download.openmmlab.com/mmclassification/v0/convnext/downstream/convnext-base_3rdparty_in21k_20220301-262fd037.pth'):
        ...
        self.weight_sum_1 = nn.Sequential(*[nn.Conv2d(embed_dim, 1, kernel_size=1, stride=1, padding=0, bias=True),nn.Sigmoid()])
        self.weight_sum_2 = nn.Sequential(*[nn.Conv2d(embed_dim, 1, kernel_size=1, stride=1, padding=0, bias=True),nn.Sigmoid()])
        self.weight_sum_3 = nn.Sequential(*[nn.Conv2d(embed_dim, 1, kernel_size=1, stride=1, padding=0, bias=True),nn.Sigmoid()])
        self.weight_sum_4 = nn.Sequential(*[nn.Conv2d(embed_dim, 1, kernel_size=1, stride=1, padding=0, bias=True),nn.Sigmoid()])
    ...
    def forward(self, x,x_other_mod):
        ...
        def _inner_forward(x,x_other_mod):
            ...
            c1=c1*w1_conc+c1_sum*w1_sum
            c2=c2*w2_conc+c2_sum*w2_sum
            c3=c3*w3_conc+c3_sum*w3_sum
            c4=c4*w4_conc+c4_sum*w4_sum
