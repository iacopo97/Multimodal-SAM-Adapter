...
from .adapter_modules_multimodal_2_adapter_branch_sequential_no_final_add import SpatialPriorModule, SpatialPriorModulenoc1, InteractionBlock, deform_inputs
...
class SAMAdapterbimodal2branchseqnofinadd(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        if self.num_mod>1:
            self.spm_other_modalities=SpatialPriorModulenoc1(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=self.modalities_ch[1])
    ...
    def forward(self, x):
        ...
        # mod_selector_gamma_ext=torch.clone(mod_selector_gamma)
        if self.num_mod>1:
            c2_other_modalities, c3_other_modalities, c4_other_modalities = self.spm_other_modalities(x_other_modalities)
