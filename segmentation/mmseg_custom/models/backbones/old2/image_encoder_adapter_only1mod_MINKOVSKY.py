...
from .adapter_modules_1mod_MINKOVSKY import SpatialPriorModule, SpatialPriorModuleSparse, InteractionBlock, deform_inputs
...
class SAMAdapterRGB1modMinkovsky(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
                 ...
                 use_extra_extractor=True, with_cp=True, drop_path_rate=0.4,drop_rate=0., drop_multimodal_path=0.2, bs=1, *args, **kwargs):
        ...
        # self.spm = SpatialPriorModule(inplanes=conv_inplane, embed_dim=embed_dim, in_ch=modalities_ch[1],with_cp=False)
        self.spm = SpatialPriorModuleSparse(inplanes=conv_inplane, embed_dim=embed_dim, in_channels=self.modalities_ch[1],with_cp=False,D=2,bs=bs)
    ...
    def forward(self, x):
        ...
        x_1mod=x[:,self.in_ch_im:]
        x_rgb=x[:,:self.in_ch_im]
        # if ('rgb' in self.modalities_name):
            # SPM forward
        c1, c2, c3, c4 = self.spm(x_1mod)
        c2, c3, c4 = self._add_level_embed(c2, c3, c4)
        c = torch.cat([c2, c3, c4], dim=1)
        
        deform_inputs1, deform_inputs2 = deform_inputs(x_1mod)
        # if self.num_mod>1:
        ...
        x_rgb, H, W = self.patch_embed(x_rgb)
        # x=x.flatten(2)
        # x=x.flatten(1,2)
        ...
        x_rgb = self.pos_drop(x_rgb + pos_embed)
        # x=x+pos_embed
        ...
        for i, layer in enumerate(self.interactions):
            ...
            x_rgb,c=layer(x_rgb, c, self.blocks[indexes[0]:indexes[-1] + 1],
                        deform_inputs1, deform_inputs2, H, W)
            outs.append(x_rgb.transpose(1, 2).view(bs, dim, H, W).contiguous())
            # outs_rgb.append(x_rgb.transpose(1, 2).view(bs, dim, H, W).contiguous())
