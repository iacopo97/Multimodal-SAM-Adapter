...
from .adapter_modules_monomodal_new_in_convnext_new import SpatialPriorModule, SpatialPriorModuleBimodal, SpatialPriorModuleMonomodal, InteractionBlock, deform_inputs
...
class SAMAdaptermonomodalNewInConvNEW(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        if ('rgb' in self.modalities_name) and self.num_mod>1:
            ...
            self.spm = SpatialPriorModuleMonomodal(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=self.in_ch_im, img_size=self.img_size,arch=arch,checkpoint=checkpoint)
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            c1, c2, c3, c4 = self.spm(x_other_modalities)
