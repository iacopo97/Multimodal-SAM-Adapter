...
class SpatialPriorModulecombinedspatial(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3):
        super().__init__()
        self.with_cp = with_cp
        ...
        self.fc_mod1=nn.Conv2d(inplanes, embed_dim//2, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc_mod2=nn.Conv2d(2*inplanes, embed_dim//2, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc_mod3=nn.Conv2d(4*inplanes, embed_dim//2, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc_mod4=nn.Conv2d(4*inplanes, embed_dim//2, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            c3 = self.conv3(c2)
            c4 = self.conv4(c3)
            mod1_t=self.fc_mod1(c1)
            mod2_t=self.fc_mod2(c2)
            mod3_t=self.fc_mod3(c3)
            mod4_t=self.fc_mod4(c4)
            c1 = self.fc1(c1)
            c2 = self.fc2(c2)
            c3 = self.fc3(c3)
            c4 = self.fc4(c4)
            # c2_t=F.interpolate(c2, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            # c3_t=F.interpolate(c3, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            # c4_t=F.interpolate(c4, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            # concat_c=torch.cat([c1,c2_t,c3_t,c4_t],dim=1)
            # concat_c=torch.cat([c1,c2,c3,c4],dim=1)
            # mod_selector_gamma1=self.mod_selector(c1,c2,c3,c4)
            # , mod_selector_gamma2, mod_selector_gamma3, mod_selector_gamma4=self.mod_selector(c1,c2,c3,c4)
            bs, dim, _, _ = c1.shape
            c1 = c1.view(bs, dim, -1).transpose(1, 2)  # 4s 2,1024, HW/4^2 view is flattening the tensor transpose therefore 2, HW/4^2, 1024
            c2 = c2.view(bs, dim, -1).transpose(1, 2)  # 8s
            c3 = c3.view(bs, dim, -1).transpose(1, 2)  # 16s
            ...
            return mod1_t,mod2_t,mod3_t,mod4_t, c1, c2, c3, c4
        # mod_selector_gamma2, mod_selector_gamma3, mod_selector_gamma4,c1, c2, c3, c4
...
class ModalitySelectorViT(nn.Module):
    def __init__(self, embed_dim=64, with_cp=False, num_mod=4):
        super().__init__()
        self.with_cp = with_cp
        self.num_mod = num_mod
        self.conv1 = nn.Sequential(*[
            nn.Conv2d(embed_dim, num_mod, kernel_size=1, stride=1, padding=0, bias=True),
            nn.ReLU(inplace=True) #add Sigmoid
        ])

    def forward(self, x,c3_orig_shape):
        ...
        def _inner_forward(x):
            ...
            # c=torch.cat([c1,c2,c3,c4],dim=1)
            # bs,_,_=x.shape
            x=x.transpose(1, 2).view(c3_orig_shape)
            mod_selector_gamma3=self.conv1(x)
            # mod_selector_gamma2=F.interpolate(mod_selector_gamma3, size=c2_orig_shape[-2:], mode='bilinear', align_corners=False)
            # mod_selector_gamma1=F.interpolate(mod_selector_gamma3, size=c1_orig_shape[-2:], mode='bilinear', align_corners=False)
            # mod_selector_gamma4=F.interpolate(mod_selector_gamma3, size=c4_orig_shape[-2:], mode='bilinear', align_corners=False)            
            # mod_selector_gamma1=mod_selector_gamma1.view(bs, self.num_mod, -1).transpose(1, 2)  # 4s bs,num_mod, HW/4^2 view is flattening the tensor transpose therefore bs, HW/4^2, num_mod
            # mod_selector_gamma2=mod_selector_gamma2.view(bs, self.num_mod, -1).transpose(1, 2) #8s
            # mod_selector_gamma3=mod_selector_gamma3.view(bs, self.num_mod, -1).transpose(1, 2) #16s
            # mod_selector_gamma4=mod_selector_gamma4.view(bs, self.num_mod, -1).transpose(1, 2) #32s
            return mod_selector_gamma3
...
class ModalityCombiner(nn.Module):
    def __init__(self, embed_dim, with_cp=False, num_mod=4):
        super().__init__()
        self.with_cp = with_cp
        self.num_mod = num_mod
        self.channel_equalizer=nn.Conv2d(embed_dim, embed_dim//2, kernel_size=1, stride=1, padding=0, bias=True)
        # self.merger=nn.Conv2d(embed_dim//2, embed_dim//4, kernel_size=1, stride=1, padding=0, bias=True)
        self.conv1 = nn.Sequential(*[
            nn.Conv2d(5*embed_dim//2, num_mod, kernel_size=1, stride=1, padding=0, bias=True),
            nn.ReLU(inplace=True) #add Sigmoid
        ])

    def forward(self, mod_1t,mod_2t,mod_3t,mod_4t,x,c1_orig_shape,c2_orig_shape,c3_orig_shape,c4_orig_shape):
        
        def _inner_forward(mod_1t,mod_2t,mod_3t,mod_4t,x, c1_orig_shape,c2_orig_shape,c3_orig_shape,c4_orig_shape):
            # c2_orig_shape=c2.shape
            # c3_orig_shape=c3.shape
            # c4_orig_shape=c4.shape
            # c2=F.interpolate(c2, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            ...
            # mod_3_comb=torch.cat([x,mod_3t],dim=1)
            # mod_3_comb=self.merger(mod_3_comb)
            x=F.interpolate(x, size=c1_orig_shape[-2:], mode='bilinear', align_corners=False)
            mod_3t=F.interpolate(mod_3t, size=c1_orig_shape[-2:], mode='bilinear', align_corners=False)
            mod_1t=F.interpolate(mod_1t, size=c1_orig_shape[-2:], mode='bilinear', align_corners=False)
            mod_2t=F.interpolate(mod_2t, size=c1_orig_shape[-2:], mode='bilinear', align_corners=False)
            mod_4t=F.interpolate(mod_3t, size=c1_orig_shape[-2:], mode='bilinear', align_corners=False)
            mod_concat=torch.cat([mod_1t,mod_2t,mod_3t,mod_4t,x],dim=1)
            mod_selector_gamma1=self.conv1(mod_concat)
            mod_selector_gamma2=F.interpolate(mod_selector_gamma1, size=c2_orig_shape[-2:], mode='bilinear', align_corners=False)
            mod_selector_gamma3=F.interpolate(mod_selector_gamma1, size=c3_orig_shape[-2:], mode='bilinear', align_corners=False)
            mod_selector_gamma4=F.interpolate(mod_selector_gamma1, size=c4_orig_shape[-2:], mode='bilinear', align_corners=False)            
            mod_selector_gamma1=mod_selector_gamma1.view(bs, self.num_mod, -1).transpose(1, 2)  # 4s bs,num_mod, HW/4^2 view is flattening the tensor transpose therefore bs, HW/4^2, num_mod
        ...
        if self.with_cp and x.requires_grad:
            outs = cp.checkpoint(_inner_forward, mod_1t,mod_2t,mod_3t,mod_4t,x,c1_orig_shape,c2_orig_shape,c3_orig_shape,c4_orig_shape)
        else:
            outs = _inner_forward(mod_1t,mod_2t,mod_3t,mod_4t,x,c1_orig_shape,c2_orig_shape,c3_orig_shape,c4_orig_shape)
        return outs