...
class SpatialPriorModuleModSelectorsinglechannel(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3):
        ...
        self.fc_mod3 = nn.Conv2d(4 * inplanes, embed_dim//2, kernel_size=1, stride=1, padding=0, bias=True) #needed for creating the spatial gamma for injector
        self.fc_mod4 = nn.Conv2d(4 * inplanes, embed_dim//2, kernel_size=1, stride=1, padding=0, bias=True)
        self.mod_selector = ModalitySelector(embed_dim=embed_dim//2, with_cp=with_cp,num_mod=1)
