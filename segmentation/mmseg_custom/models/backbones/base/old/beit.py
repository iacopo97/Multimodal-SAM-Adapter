...
class Attention(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None):
        ...
        if self.relative_position_bias_table is not None:
            ...
            relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()  # nH, Wh*Ww, Wh*Ww  Wh=H//patch_size[0], Ww=W//patch_size[1]
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.patch_shape = (img_size[0] // patch_size[0], img_size[1] // patch_size[1]) #really big therefore rel pos bias could be expensive
...
class RelativePositionBias(nn.Module):
    ...
    def __init__(self, window_size, num_heads):
        ...
        coords = torch.stack(torch.meshgrid([coords_h, coords_w]))  # 2, Wh, Ww
        coords_flatten = torch.flatten(coords, 1)  # 2, Wh*Ww
        relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # 2, Wh*Ww, Wh*Ww create relative position table assuming each pathc is an origin, therefore we will ha a 2, Wh*Ww
        relative_coords = relative_coords.permute(1, 2, 0).contiguous()  # Wh*Ww, Wh*Ww, 2
        relative_coords[:, :, 0] += window_size[0] - 1  # shift to start from 0
        relative_coords[:, :, 1] += window_size[1] - 1
        relative_coords[:, :, 0] *= 2 * window_size[1] - 1
    ...
    def __init__(self, img_size=512, patch_size=16, in_chans=3, num_classes=80, embed_dim=768,
        ...
        self.blocks = nn.ModuleList([
            Block(
                ...
                init_values=init_values, window_size=self.patch_embed.patch_shape if use_rel_pos_bias else None) # window_size=(H//patch_size[0], W//patch_size[1])	
