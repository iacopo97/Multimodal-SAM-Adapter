    ...
    def __init__(
        ...
        for i in range(depth):
            block = Block(
                ...
                input_size=(pretrained_size // patch_size, pretrained_size // patch_size), with_cp=with_cp,
            )
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        # self.learnable_embeddings = nn.Parameter(torch.ones(1, input_size, dim)*init_values)
        self.attn = Attention(
            dim,
            num_heads=num_heads,
            qkv_bias=qkv_bias,
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        rel_pos_zero_init: bool = True,
        input_size: Optional[Tuple[int, int]] = None,
        learnable_embeddings: Optional[nn.Parameter] = None,
    ) -> None:
        """
        Args:
            dim (int): Number of input channels.
...
class MFA(nn.Module):
    def __init__(self,dim,input_size,factor=14,init_values=1e-6):
        super().__init__()
        self.layer_norm=nn.LayerNorm(dim)
        self.learnable_embeddings = nn.Parameter(torch.ones(1, input_size[0], input_size[1], dim)*init_values)
        self.g3=nn.Linear(dim,int(dim/factor))
        self.g2=nn.Linear(dim,int(dim/factor))
        self.g1=nn.Linear(dim,int(dim/factor))
