...
from mmpretrain_custom.models.utils import GRN, build_norm_layer
...
class ConvNeXtBlock(BaseModule):
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            x,y=torch.split(x, (x.shape[1]//2, x.shape[1]//2), dim=1)
            shortcut = x
            shortcut2=y
            ...
            if self.linear_pw_conv:
                ...
                y = self.norm(y, data_format='channel_last')
                y = self.pointwise_conv1(y)
                y = self.act(y)
                if self.grn is not None:
                    y = self.grn(y, data_format='channel_last')
                ...
                x = self.pointwise_conv2(x)
                y = self.norm(y, data_format='channel_first')
                y = self.pointwise_conv1(y)
                y = self.act(y)

                if self.grn is not None:
                    y = self.grn(y, data_format='channel_first')
                y = self.pointwise_conv2(y)
...
class TwinConvNeXtShared(BaseBackbone):
        ...
        for i in range(self.num_stages):
            ...
            stage = Sequential(*[
                ConvNeXtBlock(
                    ...
                    use_grn=use_grn,
                    with_cp=True) for j in range(depth)
            ])
            block_idx += depth
    ...
    def forward(self, x,y):
        ...
        for i, stage in enumerate(self.stages):
            ...
            #     return x
            # x=cp.checkpoint(_inner_forward, x)
            # def _inner_forward2(y):
            #     y = self.downsample_layers[i](y)
            #     y = stage(y)
            #     # x = stage(x)
            #     return y
            # y=cp.checkpoint(_inner_forward2, y)
            x = self.downsample_layers[i](x)
            y= self.downsample_layers[i](y)
            # x = stage(x)
            # y= stage(y)
            ...
            if i in self.out_indices:
                norm_layer = getattr(self, f'norm{i}')
                if self.gap_before_final_norm:
                    gap_1 = x.mean([-2, -1], keepdim=True)
                    gap_2 = y.mean([-2, -1], keepdim=True)
                    outs.append((norm_layer(gap_1).flatten(1),norm_layer(gap_2).flatten(1))
                    )
                else:
                    outs.append((norm_layer(x),norm_layer(y)))

        return [torch.cat((out_x, out_y), dim=1) for out_x, out_y in outs]
    ...
    # def forward(self, x,y):
    #     outs_x= self.convnext(x)
    #     outs_y= self.convnext(y)
    
    #     return [torch.cat((out_x, out_y), dim=1) for out_x, out_y in zip(outs_x, outs_y)]

    def train(self, mode=True):
        super(TwinConvNeXtShared, self).train(mode)
        self._freeze_stages()
