...
class WindowedAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        qkv = F.pad(qkv, [0, W_ - W, 0, H_ - H], mode=self.pad_mode) ###########[B, C, H_, W_] in orde to make spatial dimension multiple of window size

        qkv = F.unfold(qkv, kernel_size=(self.window_size, self.window_size),
                       stride=(self.window_size, self.window_size)) #creating windows of size window_size x window_size from the qkv tensor
        B, C_kw_kw, L = qkv.shape  # L is the the num of windows
        qkv = qkv.reshape(B, C * 3, N_, L).permute(0, 3, 2, 1)  # [B, L, N_, C] 3 is used for query key and value
