...
from mmengine_custom.utils.dl_utils.parrots_wrapper import _BatchNorm

from mmpretrain_custom.registry import MODELS
from mmpretrain_custom.models.utils import (ShiftWindowMSA, resize_pos_embed,
                     resize_relative_position_bias_table, to_2tuple)

from mmseg.models.builder import BACKBONES
from mmpretrain_custom.models.backbones.base_backbone import BaseBackbone
...
class TWINSwinTransformer(BaseBackbone):
    """Swin Transformer.
    ...
    def __init__(self,
                 ...
                 patch_cfg=dict(),
                 init_cfg=None):
        super(TWINSwinTransformer, self).__init__(init_cfg=init_cfg)
        ...
        self.patch_embed_x = PatchEmbed(**_patch_cfg)
        self.patch_embed_y = PatchEmbed(**_patch_cfg)
        # self.patch_resolution = self.patch_embed.init_out_size
        self.patch_resolution = self.patch_embed_x.init_out_size

        if self.use_abs_pos_embed:
            num_patches = self.patch_resolution[0] * self.patch_resolution[1]
            # self.absolute_pos_embed = nn.Parameter(
            #     torch.zeros(1, num_patches, self.embed_dims))
            self.absolute_pos_embed_x = nn.Parameter(
                torch.zeros(1, num_patches, self.embed_dims))
        ...
        self.stages_x = ModuleList()
        self.stages_y = ModuleList()
        embed_dims = [self.embed_dims]
        for i, (depth,num_heads) in enumerate(zip(self.depths, self.num_heads)):
            ...
            stage_x = SwinBlockSequence(**_stage_cfg)
            stage_y = SwinBlockSequence(**_stage_cfg)
            self.stages_x.append(stage_x)
            self.stages_y.append(stage_y)

            dpr = dpr[depth:]
            embed_dims.append(stage_x.out_channels)
        ...
        for i in out_indices:
            if norm_cfg is not None:
                norm_layer_x = build_norm_layer(norm_cfg,
                                              self.num_features[i])[1]
                norm_layer_y = build_norm_layer(norm_cfg,self.num_features[i])[1]
                
            else:
                norm_layer_x = nn.Identity()
                norm_layer_y = nn.Identity()

            self.add_module(f'norm_x_{i}', norm_layer_x)
            self.add_module(f'norm_y_{i}', norm_layer_y)
    ...
    #     super(TWINSwinTransformer, self).init_weights()
    ...
    def _init_weights(self):
        if self.use_abs_pos_embed:
            trunc_normal_(self.absolute_pos_embed_x, std=0.02)
    ...
    #     if self.use_abs_pos_embed:
    #         trunc_normal_(self.absolute_pos_embed_1, std=0.02)
    #         trunc_normal_(self.absolute_pos_embed_2, std=0.02)

    def forward(self, x,y):
        # x,y=torch.
        x, hw_shape = self.patch_embed_x(x)
        if self.use_abs_pos_embed:
            x = x + resize_pos_embed(
                self.absolute_pos_embed_x, self.patch_resolution, hw_shape,
                self.interpolate_mode, self.num_extra_tokens)
        x = self.drop_after_pos(x)
        
        y, hw_shape_y = self.patch_embed_y(y)
        if self.use_abs_pos_embed:
            y = y + resize_pos_embed(
                self.absolute_pos_embed_y, self.patch_resolution, hw_shape_y,
                self.interpolate_mode, self.num_extra_tokens)
        ...
        for i, stage_x in enumerate(self.stages_x):
            ...
            if i in self.out_indices:
                norm_layer_x = getattr(self, f'norm_x_{i}')
                out_x = norm_layer_x(x)
                out_x = out_x.view(-1, *hw_shape,
                               self.num_features[i]).permute(0, 3, 1,
                                                             2).contiguous()
                outs_x.append(out_x)
            if stage_x.downsample is not None and not self.out_after_downsample:
                x, hw_shape = stage_x.downsample(x, hw_shape)
        
        outs_y = []
        for i, stage_y in enumerate(self.stages_y):
            y, hw_shape_y = stage_y(
                y, hw_shape_y, do_downsample=self.out_after_downsample)
            if i in self.out_indices:
                norm_layer_y = getattr(self, f'norm_y_{i}')
                out_y = norm_layer_y(y)
                out_y = out_y.view(-1, *hw_shape_y,
                               self.num_features[i]).permute(0, 3, 1,
                                                             2).contiguous()
                outs_y.append(out_y)
            if stage_y.downsample is not None and not self.out_after_downsample:
                y, hw_shape_y = stage_y.downsample(y, hw_shape_y)
    ...
    def _load_from_state_dict(self, state_dict, prefix,local_metadata,   *args,
                              **kwargs): #, prefix,local_metadata,
        """load checkpoints."""
        # Names of some parameters in has been changed.
        version = local_metadata.get('version', None)
        # version = 0.15
        if (version is None
                or version < 2) and self.__class__ is TWINSwinTransformer:
            final_stage_num = len(self.stages_x) - 1
            ...
            for k in state_dict_keys:
                if k.startswith('norm.') or k.startswith('backbone.norm.'):
                    ...
                    del state_dict[k]
        if (version is None
                or version < 3) and self.__class__ is TWINSwinTransformer:
            state_dict_keys = list(state_dict.keys())
            for k in state_dict_keys:
                if 'attn_mask' in k:
                    del state_dict[k]
    ...
    def _freeze_stages(self):
        if self.frozen_stages >= 0:
            self.patch_embed_x.eval()
            self.patch_embed_y.eval()
            for param in self.patch_embed_x.parameters():
                param.requires_grad = False
            for param in self.patch_embed_y.parameters():
                param.requires_grad = False
        ...
        for i in range(0, self.frozen_stages + 1):
            ...
            for param in m_1.parameters():
                param.requires_grad = False
            for param in m_2.parameters():
                param.requires_grad = False
        for i in self.out_indices:
            if i <= self.frozen_stages:
                for param in getattr(self, f'norm_x_{i}').parameters():
                    param.requires_grad = False
                for param in getattr(self, f'norm_y_{i}').parameters():
                    param.requires_grad = False
    # def _freeze_stages(self):
    #     if self.frozen_stages >= 0:
    ...
    def train(self, mode=True):
        super(TWINSwinTransformer, self).train(mode)
        self._freeze_stages()
    ...
    # def _prepare_abs_pos_embed(self, state_dict, prefix, *args, **kwargs):
    #     name_1 = prefix + 'absolute_pos_embed_x'
    #     name_2 = prefix + 'absolute_pos_embed_y'
    #     if name_1 not in state_dict.keys():
    #         return

    #     ckpt_pos_embed_shape = state_dict[name_1].shape
    #     if self.absolute_pos_embed_x.shape != ckpt_pos_embed_shape:
    #         from mmengine_custom.logging import MMLogger
    #         logger = MMLogger.get_current_instance()
    #         logger.info(
    #             'Resize the absolute_pos_embed shape from '
    ...
    #         ckpt_pos_embed_shape = to_2tuple(
    #             int(np.sqrt(ckpt_pos_embed_shape[1] - self.num_extra_tokens)))
    #         pos_embed_shape = self.patch_embed_x.init_out_size

    #         state_dict[name_1] = resize_pos_embed(state_dict[name_1],
    #                                             ckpt_pos_embed_shape,
    #                                             pos_embed_shape,
    #                                             self.interpolate_mode,
    #                                             self.num_extra_tokens)
    #         state_dict[name_2] = resize_pos_embed(state_dict[name_2],
    #                                             ckpt_pos_embed_shape,
    #                                             pos_embed_shape,
    #                                             self.interpolate_mode,
    #                                             self.num_extra_tokens)
    ...
    def _prepare_relative_position_bias_table(self, state_dict, prefix, *args,
        ...
        for key in all_keys:
            if 'relative_position_bias_table' in key:
                ckpt_key = prefix + key.replace('_x', '').replace('_y', '')
                if ckpt_key not in state_dict:
                    continue
                relative_position_bias_table_pretrained = state_dict[ckpt_key]
                relative_position_bias_table_current = state_dict_model[key]
                ...
                if L1 != L2:
                    ...
                    index_buffer = ckpt_key.replace('bias_table', 'index')
                    del state_dict[index_buffer]
            # if 'relative_position_bias_table_y' in key:
            #     ckpt_key = prefix + key
            #     if ckpt_key not in state_dict:
            #         continue
            #     relative_position_bias_table_pretrained_2 = state_dict[ckpt_key]
            #     relative_position_bias_table_current_2 = state_dict_model[key]
            #     L1, nH1 = relative_position_bias_table_pretrained_2.size()
            #     L2, nH2 = relative_position_bias_table_current_2.size()
            #     if L1 != L2:
            #         src_size = int(L1**0.5)
            #         dst_size = int(L2**0.5)
            #         new_rel_pos_bias = resize_relative_position_bias_table(
            #             src_size, dst_size,
            #             relative_position_bias_table_pretrained_2, nH1)
            #         from mmengine_custom.logging import MMLogger
            #         logger = MMLogger.get_current_instance()
            #         logger.info('Resize the relative_position_bias_table from '
            #                     f'{state_dict[ckpt_key].shape} to '
