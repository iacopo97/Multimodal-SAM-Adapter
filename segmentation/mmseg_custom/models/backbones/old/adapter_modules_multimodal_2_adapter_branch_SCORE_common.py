    ...
    def forward(self, query, reference_points, feat, feat_other_modalities, spatial_shapes, level_start_index):
        def _inner_forward(query, feat, feat_other_modalities):
            ...
            if self.num_mod>1:#>1
                ...
                attn_other_modalities = self.attn_other_modalities(query_n, reference_points,
                            feat_other_modalities, spatial_shapes,
                            level_start_index, None)
                query = query + self.gamma*attn + self.gamma_other_modalities*attn_other_modalities
            else:
...
class SpatialPriorModuleGeneric(nn.Module):
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            return c1, c2, c3, c4, c1_shape, c2_shape, c3_shape, c4_shape
        # , c5
...
class ModalityScore(nn.Module):
    def __init__(self, embed_dim=64, with_cp=False, num_mod=4, inter_ch=32):
        ...
        self.mix_score = nn.Sequential(*[
            nn.Conv2d(4*num_mod, num_mod, kernel_size=1, stride=1, padding=0, bias=True),
            # nn.Linear(embed_dim*num_mod,inter_ch),
            # nn.GELU(),
            # nn.Linear(inter_ch,1),
            nn.Softmax2d()
        ])

    def forward(self, c_merged,c1_shape,c2_shape,c3_shape,c4_shape):
        
        def _inner_forward(c_merged,c1_shape,c2_shape,c3_shape,c4_shape):
            mod_score=self.score(c_merged)
            dim1=c1_shape[2]*c1_shape[3]
            dim2=c2_shape[2]*c2_shape[3]
            dim3=c3_shape[2]*c3_shape[3]
            dim4=c4_shape[2]*c4_shape[3]
            mod_c1=mod_score[:,0:dim1,:].transpose(1,2).view(-1,mod_score.shape[2],c1_shape[2],c1_shape[3])
            mod_c2=mod_score[:,dim1:(dim1+dim2),:].transpose(1,2).view(-1,mod_score.shape[2],c2_shape[2],c2_shape[3])
            mod_c3=mod_score[:,(dim1+dim2):(dim1+dim2+dim3),:].transpose(1,2).view(-1,mod_score.shape[2],c3_shape[2],c3_shape[3])
            mod_c4=mod_score[:,(dim1+dim2+dim3):(dim1+dim2+dim3+dim4),:].transpose(1,2).view(-1,mod_score.shape[2],c4_shape[2],c4_shape[3])
            mod_c2=F.interpolate(mod_c2, size=c1_shape[-2:], mode='bilinear', align_corners=False)
            mod_c3=F.interpolate(mod_c3, size=c1_shape[-2:], mode='bilinear', align_corners=False)
            mod_c4=F.interpolate(mod_c4, size=c1_shape[-2:], mode='bilinear', align_corners=False)
            mod_c=torch.cat([mod_c1,mod_c2,mod_c3,mod_c4],dim=1)
            mod_score=self.mix_score(mod_c)
            bs=c1_shape[0]
            mod_score_ext2=F.interpolate(mod_score, size=c2_shape[-2:], mode='bilinear', align_corners=False).view(bs, self.num_mod, -1).transpose(1, 2)
            mod_score_ext3=F.interpolate(mod_score, size=c3_shape[-2:], mode='bilinear', align_corners=False).view(bs, self.num_mod, -1).transpose(1, 2)
            mod_score_ext4=F.interpolate(mod_score, size=c4_shape[-2:], mode='bilinear', align_corners=False).view(bs, self.num_mod, -1).transpose(1, 2)
            mod_score=mod_score.view(bs, self.num_mod, -1).transpose(1, 2)
            mod_score_fin=torch.cat([mod_score,mod_score_ext2,mod_score_ext3,mod_score_ext4],dim=1) 
            # c2_orig_shape=c2.shape
            # c3_orig_shape=c3.shape
            # c4_orig_shape=c4.shape
            # c2=F.interpolate(c2, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            ...
            # mod_selector_gamma3=F.interpolate(mod_selector_gamma1, size=c3_orig_shape[-2:], mode='bilinear', align_corners=False)
            # mod_selector_gamma4=F.interpolate(mod_selector_gamma1, size=c4_orig_shape[-2:], mode='bilinear', align_corners=False)            
            return mod_score_fin
        # , mod_selector_gamma2, mod_selector_gamma3, mod_selector_gamma4
        
        if self.with_cp and c_merged.requires_grad:
            outs = cp.checkpoint(_inner_forward, c_merged,c1_shape,c2_shape,c3_shape,c4_shape)
        else:
            outs = _inner_forward(c_merged,c1_shape,c2_shape,c3_shape,c4_shape)
        return outs
