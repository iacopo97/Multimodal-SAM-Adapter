...
class SpatialPriorModuleMonomodal(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, img_size=(1024,1024),arch='base',checkpoint='https://download.openmmlab.com/mmclassification/v0/convnext/downstream/convnext-base_3rdparty_in21k_20220301-262fd037.pth'):
        super().__init__()
        self.with_cp = with_cp
        # inplanes=64
    ...
    def _init_weights(self, m):
        if isinstance(m, nn.Linear):
            ...
            if m.bias is not None:
                m.bias.data.zero_()
    def forward(self, x_other_mod):
        
        def _inner_forward(x_other_mod):
            c1,c2,c3,c4=self.conv(x_other_mod)
            c1 = self.fc1(c1)
            c2 = self.fc2(c2)
            c3 = self.fc3(c3)
            c4 = self.fc4(c4)
        ...
        if self.with_cp and x_other_mod.requires_grad:
            outs = cp.checkpoint(_inner_forward, x_other_mod)
        else:
            outs = _inner_forward(x_other_mod)
        return outs
