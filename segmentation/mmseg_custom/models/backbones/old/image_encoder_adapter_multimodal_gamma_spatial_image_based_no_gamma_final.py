...
from .adapter_modules_multimodal_gamma_spatial_image_based_no_gamma_final import SpatialPriorModule, SpatialPriorModulewithGammaSpatial, InteractionBlock, deform_inputs
...
class SAMAdaptermultimodalselectorgamma_nogammafinal(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        super().__init__(num_heads=num_heads, pretrained=pretrained, #,
                         with_cp=True,*args,**kwargs) #with_cp=with_cp *args, **kwargs
    ...
    def forward(self, x):
        ...
        if ('rgb' in self.modalities_name):
            # SPM forward
            c1, c2, c3, c4, mod_selector_gamma1, mod_selector_gamma2, mod_selector_gamma3, mod_selector_gamma4, mod_selector_gamma_inj = self.spm_gamma_spatial(x)
        ...
        if self.num_mod>1:
            ...
            for i in range(1,len(self.modalities_name)):
                ...
                c_other_modalities_t = mod_selector_gamma[:,:,i].unsqueeze(-1)*torch.cat([c2_other_modalities_t, c3_other_modalities_t, c4_other_modalities_t], dim=1)
                if i>1:
        ...
        for i, layer in enumerate(self.interactions):
            ...
            if self.num_mod>1:
                c=mod_selector_gamma[:,:,0].unsqueeze(-1)*c
        ...
        if ('rgb' in self.modalities_name) and (self.num_mod==1):
            ...
            c1=mod_selector_gamma1[:,:,0].unsqueeze(-1)*c1+c1_other_modalities_cumulative
        ...
        return [f1, f2, f3, f4], mod_selector_gamma1, mod_selector_gamma2, mod_selector_gamma3,mod_selector_gamma4,mod_selector_gamma_inj
