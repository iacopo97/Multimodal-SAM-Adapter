...
class SpatialPriorModulewithGammaSpatial(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, num_mod=4):
        ...
        self.norm_mod1=nn.SyncBatchNorm(self.num_mod)
        self.norm_mod2=nn.SyncBatchNorm(self.num_mod)
        self.norm_mod3=nn.SyncBatchNorm(self.num_mod)
        self.norm_mod4=nn.SyncBatchNorm(self.num_mod)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            mod_selector_gamma1_t=self.norm_mod1(mod_selector_gamma1_t)
            mod_selector_gamma2_t=self.norm_mod2(mod_selector_gamma2_t)
            mod_selector_gamma3_t=self.norm_mod3(mod_selector_gamma3_t)
            mod_selector_gamma4_t=self.norm_mod4(mod_selector_gamma4_t)
            ...
            mod_selector_t=torch.cat((mod_selector_gamma1_t,mod_selector_gamma2_t,mod_selector_gamma3_t,mod_selector_gamma4_t),dim=1)
            mod_selector_t=self.merger(mod_selector_t)
