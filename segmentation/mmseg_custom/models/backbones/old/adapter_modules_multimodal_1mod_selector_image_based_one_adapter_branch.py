...
class SpatialPriorModulewithGammaSpatial(nn.Module):
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            # c3=c3[:,:-self.num_mod,:,:]
            # c3=c3[:,:-self.num_mod,:,:]
            mod_selector_gamma1=F.interpolate(mod_selector_gamma4, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            mod_selector_gamma2=F.interpolate(mod_selector_gamma4, size=c2.shape[-2:], mode='bilinear', align_corners=False)
            mod_selector_gamma3=F.interpolate(mod_selector_gamma4, size=c3.shape[-2:], mode='bilinear', align_corners=False)
            c4=c4[:,:-self.num_mod,:,:]
            bs, dim, _, _ = c1.shape
            c1 = c1.view(bs, dim, -1).transpose(1, 2)  # 4s 2,1024, HW/4^2 view is flattening the tensor transpose therefore 2, HW/4^2, 1024
            c2 = c2.view(bs, dim, -1).transpose(1, 2)  # 8s
...
#     def __init__(self, embed_dim=384, with_cp=False, num_mod=4):
#         super().__init__()
#         self.with_cp = with_cp
#         self.num_mod = num_mod
...
#         #     nn.ReLU(inplace=True)
#         # ])
#         self.fc1 = nn.Conv2d(embed_dim, num_mod, kernel_size=1, stride=1, padding=0, bias=True)
#         self.fc2 = nn.Conv2d(2 * inplanes, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
#         self.fc3 = nn.Conv2d(4 * inplanes, embed_dim, kernel_size=1, stride=1, padding=0, bias=True) #needed for creating the spatial gamma for injector
#         # self.fc3 = nn.Conv2d(4 * inplanes, embed_dim+self.num_mod, kernel_size=1, stride=1, padding=0, bias=True) #needed for creating the spatial gamma for injector
#         self.fc4 = nn.Conv2d(4 * inplanes, embed_dim+self.num_mod, kernel_size=1, stride=1, padding=0, bias=True)
