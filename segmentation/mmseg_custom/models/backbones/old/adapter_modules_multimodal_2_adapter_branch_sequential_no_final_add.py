...
class InteractionBlock(nn.Module):
    def __init__(self, dim, num_heads=6, n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),
        ...
        self.modalities_name=modalities_name
        self.num_mod=num_mod
        self.extra_extractors_presence=extra_extractor
    ...
    def forward(self, x, c, blocks, deform_inputs1, deform_inputs2, H, W, c_other_mod):
        x = self.injector(query=x, reference_points=deform_inputs1[0],
                          feat=c, feat_other_modalities=c_other_mod, spatial_shapes=deform_inputs1[1],
                          level_start_index=deform_inputs1[2])
        for idx, blk in enumerate(blocks):
            x = blk(x, H, W)
        if not self.extra_extractors_presence:
            c, c_other_mod = self.extractor(query=c, query_other_modalities=c_other_mod, reference_points=deform_inputs2[0],
...
class SpatialPriorModulenoc1(nn.Module):
