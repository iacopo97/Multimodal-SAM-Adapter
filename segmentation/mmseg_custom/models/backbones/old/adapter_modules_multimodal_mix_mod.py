...
class SpatialPriorModuleBimodal(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, img_size=(1024,1024)):
        super().__init__()
        self.with_cp = with_cp
        ...
        self.fc1 = nn.Conv2d(2*inplanes, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc2 = nn.Conv2d(4 * inplanes, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc3 = nn.Conv2d(8 * inplanes, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.smart_fusion = RoadFormer2Neck(in_channels=[inplanes*2,inplanes*4,inplanes*8,inplanes*8],
                 out_channels=[inplanes*2,inplanes*4,inplanes*8,inplanes*8],
                 layer = len([inplanes*2,inplanes*4,inplanes*8,inplanes*8]),
