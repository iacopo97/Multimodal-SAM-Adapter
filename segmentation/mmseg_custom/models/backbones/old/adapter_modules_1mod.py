...
# global COUNTER
# COUNTER=0
# def save_tensor_as_image(tensor, filename,H,W):
#     # Ensure the tensor is on the CPU and convert to numpy
#     COUNTER=COUNTER+1
#     tensor = tensor.cpu().detach().numpy()
#     bs,_,dim= tensor.shape
#     c2_size = H*16/8
#     c2 = tensor[:, 0:c2.size(1), :]
#     c3 = tensor[:, c2.size(1):c2.size(1) + c3.size(1), :]
#     c4 =tensor[:, c2.size(1) + c3.size(1):, :]

#     c2 = c2.transpose(1, 2).view(bs, dim, H * 2, W * 2).contiguous()
#     c3 = c3.transpose(1, 2).view(bs, dim, H, W).contiguous()
#     c4 = c4.transpose(1, 2).view(bs, dim, H // 2, W // 2).contiguous()
#     # Calculate the mean channel-wise
...
#     # Select the first channel and convert to PIL Image
#     for i in range(bs):
#         img = Image.fromarray(tensor[0])
        
#         # Save the image
#         img.save("/media/data4/sora/projects/Multimodal/vit-adapter-mm/segmentation/attention_maps/"+str(COUNTER)+filename)
...
class InteractionBlock(nn.Module):
    ...
    def forward(self, x, c, blocks, deform_inputs1, deform_inputs2, H, W):
        # save_tensor_as_image(c, "c_before_injector",H,W)
