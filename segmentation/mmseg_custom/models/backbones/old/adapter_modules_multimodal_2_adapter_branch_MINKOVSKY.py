...
class SpatialPriorModuleSparse(nn.Module):
    def __init__(self, inplanes=64, embed_dim=1024, spatial_dim=1024, in_channels=1,with_cp=False,D=2,bs=2):
        super().__init__()
        self.with_cp = with_cp
        self.coordinates = dense_coordinates(torch.Size([bs, in_channels, spatial_dim,spatial_dim]))
        self.sparse = MinkowskiToSparseTensor(remove_zeros=True,coordinates=self.coordinates)
        self.stem_sparse = nn.Sequential(*[
            ME.MinkowskiConvolution(in_channels, inplanes, kernel_size=3, stride=2, dilation=1, bias=False, dimension=D),
            # nn.Conv2d(in_ch, inplanes, kernel_size=3, stride=2, padding=1, bias=False),
            ME.MinkowskiSyncBatchNorm(inplanes),
            ME.MinkowskiReLU(),
        ...
        self.dense1 = MinkowskiToDenseTensor(torch.Size([bs, embed_dim, spatial_dim//4,spatial_dim//4]))
        self.dense2 = MinkowskiToDenseTensor(torch.Size([bs, embed_dim, spatial_dim//8,spatial_dim//8]))
        self.dense3 = MinkowskiToDenseTensor(torch.Size([bs, embed_dim, spatial_dim//16,spatial_dim//16]))
        self.dense4 = MinkowskiToDenseTensor(torch.Size([bs, embed_dim, spatial_dim//32,spatial_dim//32]))
