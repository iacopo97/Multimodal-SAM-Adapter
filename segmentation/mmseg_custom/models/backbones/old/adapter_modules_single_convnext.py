...
from .base.convnext import ConvNeXt
...
class FFRM(BaseModule):
    ...
    def __init__(self, in_chan, out_chan, norm=None,with_cp=False):
...
class GFFM(nn.Module):
    ...
    def __init__(self, feat_scale,dim,with_cp=False):
        super().__init__()
        self.gammax = Scale(0)
        self.gammay = Scale(0)
        self.with_cp=with_cp
...
class MobileNetV2(nn.Module):
    def __init__(self, in_channel,out_channel,with_cp=False):
...
class RoadFormer2Neck(BaseModule):   
    def __init__(self,
        ...
        for i in range(self.layer):            
            ...
            enhance_block = FFRM(old_channel, new_channel, norm=norm_cfg,with_cp=with_cp)
        ...
        self.global_feature_encoder_rgb = ModuleList([
            GFE(dim=ch // 2, num_heads=8, ffn_expansion_factor=2, qkv_bias=False,groups= 32,with_cp=with_cp)
        ...
        self.local_feature_encoder_rgb = ModuleList([
            MobileNetV2(in_channel = ch, out_channel = ch,with_cp=with_cp)
        ...
        for i in range(self.num_input_levels):            
            old_channel = in_channels[i]
            new_channel = out_channels[i]            
            ca_block = CA(old_channel, new_channel, norm=norm_cfg,with_cp=with_cp)
        ...
        for i in range(self.layer):
            fuse_block = GFFM(feat_scales[i],self.in_channels1[i],with_cp=with_cp)
...
class SpatialPriorModuleSinglemodal(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, img_size=(1024,1024),arch='base',checkpoint='https://download.openmmlab.com/mmclassification/v0/convnext/downstream/convnext-base_3rdparty_in21k_20220301-262fd037.pth'):
        ...
        #          img_scale=(img_size, img_size),
        #          norm_cfg=dict(type='GN', num_groups=32),with_cp=False)
