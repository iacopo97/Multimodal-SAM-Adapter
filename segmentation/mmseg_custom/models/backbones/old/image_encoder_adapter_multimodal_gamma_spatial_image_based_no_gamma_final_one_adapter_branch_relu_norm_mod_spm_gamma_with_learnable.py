...
from .adapter_modules_multimodal_gamma_spatial_image_based_no_gamma_final_one_adapter_branch_modalitiesspm import SpatialPriorModuleModalities, SpatialPriorModulewithGammaSpatial, InteractionBlock, deform_inputs
...
class SAMAdaptermultimodalselectorgamma_nogammafinalonebranchRelunormsumgammalearnable(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        if ('rgb' in self.modalities_name):
            ...
            ###################       
        self.level_embed = nn.Parameter(torch.zeros(3*self.num_mod, embed_dim))
        ...
        self.gamma_1_RGB=nn.Parameter(gamma_init_values * torch.ones((embed_dim)), requires_grad=True)
        self.gamma_2_RGB=nn.Parameter(gamma_init_values * torch.ones((embed_dim)), requires_grad=True)
        if self.num_mod>1:
            ...
            for i in range(1,len(self.modalities_name)):
                self.spm_other_modalities.update({f"spm_{self.modalities_name[i]}":SpatialPriorModuleModalities(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=self.modalities_ch[i])})
    ...
    def _add_level_embed(self, c2, c3, c4):
        global COUNTER
        c2 = c2 + self.level_embed[COUNTER].type_as(c2)
        COUNTER+=1
        c3 = c3 + self.level_embed[COUNTER].type_as(c3)
        COUNTER+=1
        c4 = c4 + self.level_embed[COUNTER].type_as(c4) 
        COUNTER+=1
    ...
    def forward(self, x):
        ...
        # mod_selector_gamma_ext=torch.clone(mod_selector_gamma)
        # c2_4_other_modalities_cumulative_list=list()
        # c1_other_modalities_cumulative_list=list()
        if self.num_mod>1:
            ...
            for i in range(1,len(self.modalities_name)):
                ...
                c1_other_modalities_t, c2_other_modalities_t, c3_other_modalities_t, c4_other_modalities_t, mod_selector_gamma1_t, mod_selector_gamma2_t,mod_selector_gamma3_t,mod_selector_gamma4_t = self.spm_other_modalities[f"spm_{self.modalities_name[i]}"](x_other_modalities[:,modalities_ch_cumsum[i-1]:modalities_ch_cumsum[i]])
                c1_other_modalities_t, c2_other_modalities_t, c3_other_modalities_t, c4_other_modalities_t = mod_selector_gamma1_t.unsqueeze(-1)*mod_selector_gamma1[:,:,i].unsqueeze(-1)*c1_other_modalities_t, mod_selector_gamma2_t.unsqueeze(-1)*mod_selector_gamma2[:,:,i].unsqueeze(-1)*c2_other_modalities_t, mod_selector_gamma3_t.unsqueeze(-1)*mod_selector_gamma3[:,:,i].unsqueeze(-1)*c3_other_modalities_t, mod_selector_gamma4_t.unsqueeze(-1)*mod_selector_gamma4[:,:,i].unsqueeze(-1)*c4_other_modalities_t
                ...
                c2_4_other_modalities_cumulative+=self.gamma_2_other_modalities[f"gamma_2_{self.modalities_name[i]}"]*self.multimodal_drop_path(c_other_modalities_t)
                c1_other_modalities_cumulative+=self.gamma_1_other_modalities[f"gamma_1_{self.modalities_name[i]}"]*self.multimodal_drop_path(c1_other_modalities_t)
            c=self.gamma_2_RGB*c+c2_4_other_modalities_cumulative
            c1=self.gamma_1_RGB*c1+c1_other_modalities_cumulative    
