...
class SpatialPriorModuleBimodal(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, img_size=(1024,1024),arch='base',checkpoint='https://download.openmmlab.com/mmclassification/v0/convnext/downstream/convnext-base_3rdparty_in21k_20220301-262fd037.pth'):
        ...
        self.fc2 = nn.Conv2d(inplanes*2, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc2.apply(self._init_weights)
        self.fc3 = nn.Conv2d(inplanes*4, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc3.apply(self._init_weights)
        self.fc4 = nn.Conv2d(inplanes*4, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        self.fc2_other_mod = nn.Conv2d(inplanes*2, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc2_other_mod.apply(self._init_weights)
        self.fc3_other_mod = nn.Conv2d(inplanes*4, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc3_other_mod.apply(self._init_weights)
        self.fc4_other_mod = nn.Conv2d(inplanes*4, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
