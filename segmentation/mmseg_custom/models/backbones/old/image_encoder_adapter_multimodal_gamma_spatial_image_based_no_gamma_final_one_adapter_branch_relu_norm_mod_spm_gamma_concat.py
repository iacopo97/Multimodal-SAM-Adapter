...
from .adapter_modules_multimodal_gamma_spatial_image_based_no_gamma_final_one_adapter_branch import SpatialPriorModule, SpatialPriorModulewithGammaSpatial, InteractionBlock, deform_inputs
...
class SAMAdaptermultimodalselectorgamma_nogammafinalonebranchRelunormsum_concat(ImageEncoderViT):
    def __init__(self, pretrain_size=1024 #1024
        ...
        self.merger_1=nn.Linear(2*embed_dim, embed_dim)
        self.merger_2_4=nn.Linear(2*embed_dim, embed_dim)
        # self.merger_3=nn.Conv2d(2*embed_dim, embed_dim, 1)
        # self.merger_4=nn.Conv2d(2*embed_dim, embed_dim, 1)
        self.merger_1.apply(self.init_weights)
        self.merger_2_4.apply(self.init_weights)
        # self.merger_3.apply(self.init_weights)
        # self.merger_4.apply(self.init_weights)
    ...
    def forward(self, x):
        ...
        if self.num_mod>1:
            ...
            for i in range(1,len(self.modalities_name)):
                ...
                c2_4_other_modalities_cumulative+=self.multimodal_drop_path(c_other_modalities_t)
                c1_other_modalities_cumulative+=self.multimodal_drop_path(c1_other_modalities_t)
            # c=c+c2_4_other_modalities_cumulative
            # c1=c1+c1_other_modalities_cumulative
            c_all_modals=torch.cat([c,c2_4_other_modalities_cumulative],dim=2)
            c1_all_modals=torch.cat([c1,c1_other_modalities_cumulative],dim=2)
            # c_all_modals=c_all_modals.transpose(1,2)
            # c2_all_modals = c_all_modals[:, 0:c2.size(1), :]
            # c3_all_modals = c_all_modals[:, c2.size(1):c2.size(1) + c3.size(1), :]
            # c4_all_modals = c_all_modals[:, c2.size(1) + c3.size(1):, :]

            # c2_all_modals = c2_all_modals.transpose(1, 2).view(bs, 2*dim, H * 2, W * 2).contiguous()
            # c3_all_modals = c3_all_modals.transpose(1, 2).view(bs, 2*dim, H, W).contiguous()
            # c4_all_modals = c4_all_modals.transpose(1, 2).view(bs, 2*dim, H // 2, W // 2).contiguous()
            # c1_all_modals=c1_all_modals.transpose(1,2).view(bs, 2*dim, 4*H, 4*W).contiguous()  
            # c2=self.merger_2(c2_all_modals)
            # c3=self.merger_3(c3_all_modals)
            # c4=self.merger_4(c4_all_modals)
            # c2 = c2.view(bs, dim, -1).transpose(1, 2)  # 8s
            # c3 = c3.view(bs, dim, -1).transpose(1, 2)  # 16s
            # c4 = c4.view(bs, dim, -1).transpose(1, 2)  # 32s
            # c=torch.cat([c2,c3,c4],dim=1)
            c1=self.merger_1(c1_all_modals)
            c=self.merger_2_4(c_all_modals)
