...
class SpatialPriorModulewithGammaSpatial(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, num_mod=4):
        ...
        self.stem = nn.Sequential(*[
            nn.Conv2d(in_channels, inplanes+self.num_mod, kernel_size=3, stride=2, padding=1, bias=False),
            nn.SyncBatchNorm(inplanes+self.num_mod),
            nn.ReLU(inplace=True),
            nn.Conv2d(inplanes+self.num_mod, inplanes+self.num_mod, kernel_size=3, stride=1, padding=1, bias=False),
            nn.SyncBatchNorm(inplanes+self.num_mod),
            nn.ReLU(inplace=True),
            nn.Conv2d(inplanes+self.num_mod, inplanes+self.num_mod, kernel_size=3, stride=1, padding=1, bias=False),
            nn.SyncBatchNorm(inplanes+self.num_mod),
        ...
        self.conv2 = nn.Sequential(*[
            nn.Conv2d(inplanes+self.num_mod, 2 * inplanes +self.num_mod, kernel_size=3, stride=2, padding=1, bias=False),
            nn.SyncBatchNorm(2 * inplanes +self.num_mod),
            nn.ReLU(inplace=True)
        ])
        self.conv3 = nn.Sequential(*[
            nn.Conv2d(2 * inplanes +self.num_mod, 4 * inplanes +self.num_mod, kernel_size=3, stride=2, padding=1, bias=False), 
            nn.SyncBatchNorm(4 * inplanes +self.num_mod),
            nn.ReLU(inplace=True)
        ])
        self.conv4 = nn.Sequential(*[
            nn.Conv2d(4 * inplanes +self.num_mod, 4 * inplanes +self.num_mod, kernel_size=3, stride=2, padding=1, bias=False),
            nn.SyncBatchNorm(4 * inplanes +self.num_mod),
            nn.ReLU(inplace=True)
        ])
        self.fc1 = nn.Conv2d(inplanes, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        ...
        # self.layer_norm1 = partial(nn.LayerNorm, eps=1e-6)(self.num_mod)
        # self.layer_norm2 = partial(nn.LayerNorm, eps=1e-6)(self.num_mod)
        # self.layer_norm3 = partial(nn.LayerNorm, eps=1e-6)(self.num_mod)
        # self.layer_norm4 = partial(nn.LayerNorm, eps=1e-6)(self.num_mod)
        # self.batch_norm1=nn.SyncBatchNorm(self.num_mod)
        # self.batch_norm2=nn.SyncBatchNorm(self.num_mod)
        # self.batch_norm3=nn.SyncBatchNorm(self.num_mod)
        # self.batch_norm4=nn.SyncBatchNorm(self.num_mod)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            # mod_selector_gamma1=self.batch_norm1(mod_selector_gamma1)
            # mod_selector_gamma2=self.batch_norm2(mod_selector_gamma2)
            # mod_selector_gamma3=self.batch_norm3(mod_selector_gamma3)
            # mod_selector_gamma4=self.batch_norm4(mod_selector_gamma4)
            # mod_selector_gamma3=c3[:,-self.num_mod:,:,:] # bs, num_mod, H/16, W/16
            ...
            # mod_selector_gamma1=self.act1(mod_selector_gamma1)
            # mod_selector_gamma2=self.act2(mod_selector_gamma2)  
            # mod_selector_gamma3=self.act3(mod_selector_gamma3)
            # mod_selector_gamma4=self.act4(mod_selector_gamma4)
