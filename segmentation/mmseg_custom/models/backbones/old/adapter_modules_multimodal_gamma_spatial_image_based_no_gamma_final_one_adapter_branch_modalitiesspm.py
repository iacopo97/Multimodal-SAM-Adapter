...
class SpatialPriorModuleModalities(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, num_mod=4):
        super().__init__()
        self.with_cp = with_cp
        self.num_mod = num_mod
        ...
        self.fc1 = nn.Conv2d(inplanes, embed_dim+1, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc2 = nn.Conv2d(2 * inplanes, embed_dim+1, kernel_size=1, stride=1, padding=0, bias=True)
        self.fc3 = nn.Conv2d(4 * inplanes, embed_dim+1, kernel_size=1, stride=1, padding=0, bias=True) #needed for creating the spatial gamma for injector
        # self.fc3 = nn.Conv2d(4 * inplanes, embed_dim+self.num_mod, kernel_size=1, stride=1, padding=0, bias=True) #needed for creating the spatial gamma for injector
        self.fc4 = nn.Conv2d(4 * inplanes, embed_dim+1, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            c3 = self.fc3(c3)
            c4 = self.fc4(c4)
            mod_selector_gamma1=c1[:,-1:,:,:] # bs, num_mod, H/4, W/4
            mod_selector_gamma2=c2[:,-1:,:,:] # bs, num_mod, H/8, W/8
            mod_selector_gamma3=c3[:,-1:,:,:] # bs, num_mod, H/16, W/16
            # mod_selector_gamma3=c3[:,-self.num_mod:,:,:] # bs, num_mod, H/16, W/16
            mod_selector_gamma4=c4[:,-1:,:,:] # bs, num_mod, H/32, W/32
            # mod_selector_gamma_inj=c3[:,-self.num_mod:,:,:] # bs, num_mod, H/16, W/16       
            c1=c1[:,:-1,:,:]
            c2=c2[:,:-1,:,:]
            c3=c3[:,:-1,:,:]
