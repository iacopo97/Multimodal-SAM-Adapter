...
class SpatialPriorModulewithGammaSpatial(nn.Module):
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            mod_selector_gamma2=mod_selector_gamma2+F.interpolate(mod_selector_gamma3, size=c2.shape[-2:], mode='bilinear', align_corners=False)
            mod_selector_gamma1=mod_selector_gamma1+F.interpolate(mod_selector_gamma2, size=c1.shape[-2:], mode='bilinear', align_corners=False) 
            c1=c1[:,:-self.num_mod,:,:]
            c2=c2[:,:-self.num_mod,:,:]
            c3=c3[:,:-self.num_mod,:,:]
            # c3=c3[:,:-self.num_mod,:,:]
