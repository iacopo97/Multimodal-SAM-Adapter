...
class SpatialPriorModulewithGammaSpatial(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, num_mod=4):
        ...
        self.upconv2 = nn.ConvTranspose2d(self.num_mod, self.num_mod, 2,2)
        self.upconv3 = nn.ConvTranspose2d(self.num_mod, self.num_mod, 4,4)
        self.upconv4 = nn.ConvTranspose2d(self.num_mod, self.num_mod, 8,8)
        # self.downconv2 = nn.Conv2d(self.num_mod, self.num_mod, kernel_size=3, stride=2, padding=1, bias=False)
        # self.downconv3 = nn.Conv2d(self.num_mod, self.num_mod, kernel_size=3, stride=2, padding=1, bias=False)
        # self.downconv4 = nn.Conv2d(self.num_mod, self.num_mod, kernel_size=3, stride=2, padding=1, bias=False)
        self.downconv2 = nn.Sequential(*[
            nn.Conv2d(self.num_mod, self.num_mod, kernel_size=3, stride=2, padding=1, bias=False),
            nn.ReLU(inplace=True)
        ])
        ...
        self.merger = nn.Conv2d(4*self.num_mod, self.num_mod, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            c3 = self.fc3(c3)
            c4 = self.fc4(c4)
            # mod_selector_gamma1_t=c1[:,-self.num_mod:,:,:] # bs, num_mod, H/4, W/4
            # mod_selector_gamma2_t=c2[:,-self.num_mod:,:,:] # bs, num_mod, H/8, W/8
            # mod_selector_gamma3_t=c3[:,-self.num_mod:,:,:] # bs, num_mod, H/16, W/16
            # # mod_selector_gamma3=c3[:,-self.num_mod:,:,:] # bs, num_mod, H/16, W/16
            # mod_selector_gamma4_t=c4[:,-self.num_mod:,:,:] # bs, num_mod, H/32, W/32
            # # mod_selector_gamma_inj=c3[:,-self.num_mod:,:,:] # bs, num_mod, H/16, W/16
            # # mod_selector_gamma2_t=F.interpolate(mod_selector_gamma2_t, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            # # mod_selector_gamma3_t=F.interpolate(mod_selector_gamma3_t, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            # # mod_selector_gamma4_t=F.interpolate(mod_selector_gamma4_t, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            # mod_selector_gamma2_t=self.upconv2(mod_selector_gamma2_t)
            # mod_selector_gamma3_t=self.upconv3(mod_selector_gamma3_t)
            # mod_selector_gamma4_t=self.upconv4(mod_selector_gamma4_t)
            # mod_selector_t=torch.cat((mod_selector_gamma1_t,mod_selector_gamma2_t,mod_selector_gamma3_t),dim=1)
            ...
            # mod_selector_gamma3_t=F.interpolate(mod_selector_gamma3_t, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            # mod_selector_gamma4_t=F.interpolate(mod_selector_gamma4_t, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            mod_selector_gamma2=self.upconv2(mod_selector_gamma2)
            mod_selector_gamma3=self.upconv3(mod_selector_gamma3)
            mod_selector_gamma4=self.upconv4(mod_selector_gamma4)
            mod_selector_t=torch.cat((mod_selector_gamma1,mod_selector_gamma2,mod_selector_gamma3,mod_selector_gamma4),dim=1)
            mod_selector_gamma1=self.merger(mod_selector_t)
            # mod_selector_gamma1=mod_selector_t
            # mod_selector_gamma4=F.interpolate(mod_selector_gamma1, size=c4.shape[-2:], mode='bilinear', align_corners=False)
            # mod_selector_gamma3=F.interpolate(mod_selector_gamma1, size=c3.shape[-2:], mode='bilinear', align_corners=False)
            # mod_selector_gamma2=F.interpolate(mod_selector_gamma1, size=c2.shape[-2:], mode='bilinear', align_corners=False)
            mod_selector_gamma1=self.relu_mod1(mod_selector_gamma1)
            mod_selector_gamma2=self.downconv2(mod_selector_gamma1)
            mod_selector_gamma3=self.downconv3(mod_selector_gamma2)
            mod_selector_gamma4=self.downconv4(mod_selector_gamma3)
