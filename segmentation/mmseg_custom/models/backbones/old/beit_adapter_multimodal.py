...
from .adapter_modules_multimodal_stoch_depth_end_and_inj_ext import SpatialPriorModule, deform_inputs
from .adapter_modules_multimodal_stoch_depth_end_and_inj_ext import InteractionBlockWithCls as InteractionBlock
_logger = logging.getLogger(__name__)

COUNTER=0
@BACKBONES.register_module()
class BEiTAdaptermultimodal(BEiT):
    def __init__(self,pretrain_size=224, conv_inplane=64, n_points=4, modalities_name=['rgb','depth','lidar','event'], modalities_ch=[3,3,3,1], deform_num_heads=6,
                 init_values=0., gamma_init_values=0.,cffn_ratio=0.25, deform_ratio=1.0, with_cffn=True,
                 interaction_indexes=None, add_vit_feature=True, with_cp=False, drop_path_rate=0.4,drop_rate=0., drop_multimodal_path=0.2,*args, **kwargs):
        ...
        if ('rgb' in self.modalities_name):
            ...
            ###################       
        # self.level_embed = nn.Parameter(torch.zeros(3*self.num_mod, embed_dim).type)
        ...
        # if num_mod==1:
        #     self.spm = SpatialPriorModule(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=in_ch_im)
        #     self.up = nn.ConvTranspose2d(embed_dim, embed_dim, 2, 2)
        # for i in self.modalities_name[1:]:
        #     locals()['self.spm_'+i] = SpatialPriorModule(inplanes=conv_inplane, embed_dim=embed_dim, with_cp=False, in_channels=self.modalities_ch[self.modalities_name.index(i)])
    ...
    def forward(self, x):
        ...
        # x=x.flatten(1,2)
        bs, n, dim = x.shape
        cls = self.cls_token.expand(bs, -1, -1).type_as(x)  # stole cls_tokens impl from Phil Wang, thanks
        ...
        for i, layer in enumerate(self.interactions):
            ...
            if self.num_mod>1:
                x,c,c2_4_other_modalities, cls=layer(x, c, cls, self.blocks[indexes[0]:indexes[-1] + 1],
                        deform_inputs1, deform_inputs2, H, W, c2_4_other_modalities)
            elif ('rgb' in self.modalities_name) and self.num_mod==1:
                x,c,_, cls=layer(x, c, cls, self.blocks[indexes[0]:indexes[-1] + 1],
        ...
        # Final Norm
        f1 = self.norm1(c1).type_as(c1)
        f2 = self.norm2(c2).type_as(c2)
        f3 = self.norm3(c3).type_as(c3)
        f4 = self.norm4(c4).type_as(c4)
        return [f1, f2, f3, f4]
