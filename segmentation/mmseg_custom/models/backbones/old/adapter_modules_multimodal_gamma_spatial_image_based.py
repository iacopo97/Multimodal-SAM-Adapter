...
class Extractor_multimodal(nn.Module):
    ...
    def forward(self, query, query_other_modalities,reference_points, feat, spatial_shapes, level_start_index, H, W, mod_selector_gamma_ext):
        
        def _inner_forward(query, query_other_modalities, feat, mod_selector_gamma_ext):
                    ...
                    for j in range(0,len(self.modalities_name)):
                        if self.modalities_name[i]!=self.modalities_name[j]:
                            if self.modalities_name[i]=='rgb':
                                cum_extraction_primary_modality = cum_extraction_primary_modality + attn + self.drop_multimodal(mod_selector_gamma_ext[:,:,j].unsqueeze(-1).type_as(attn_cum[j-1])*attn_cum[j-1])
                            ...
                            #     # pass
                                ...
                                cum_extraction_other_modalities_t[i-1] = cum_extraction_other_modalities_t[i-1] + attn_cum[i-1]+self.drop_multimodal(mod_selector_gamma_ext[:,:,j].unsqueeze(-1).type_as(attn)*attn_cum[j-1])
        ...
        if self.with_cp and query.requires_grad:
            ...
            query, query_other_modalities = cp.checkpoint(_inner_forward, query, query_other_modalities, feat, mod_selector_gamma_ext)
            # print(prof.key_averages().table(sort_by="cuda_time_total", row_limit=10))
        else:
            query, query_other_modalities = _inner_forward(query, query_other_modalities, feat, mod_selector_gamma_ext)
...
class InteractionBlock(nn.Module):
    ...
    def forward(self, x, c, blocks, deform_inputs1, deform_inputs2, H, W, c2_4_other_modalities, mod_selector_gamma_inj,mod_selector_gamma_ext):
        ...
        c, c2_4_other_modalities = self.extractor(query=c, query_other_modalities=c2_4_other_modalities, reference_points=deform_inputs2[0],
                           ...
                           level_start_index=deform_inputs2[2], H=H, W=W, mod_selector_gamma_ext=mod_selector_gamma_ext)
...
class SpatialPriorModulewithGammaSpatial(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, num_mod=4):
        ...
        self.fc3 = nn.Conv2d(4 * inplanes, embed_dim+self.num_mod, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            mod_selector_gamma3=mod_selector_gamma3.view(bs, self.num_mod, -1).transpose(1, 2)
            mod_selector_gamma4=mod_selector_gamma4.view(bs, self.num_mod, -1).transpose(1, 2)
