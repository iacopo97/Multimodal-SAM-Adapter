...
class SpatialPriorModuleBimodal(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, img_size=(1024,1024),arch='base',checkpoint='https://download.openmmlab.com/mmclassification/v0/convnext/downstream/convnext-base_3rdparty_in21k_20220301-262fd037.pth'):
        ...
        #     nn.ReLU(inplace=True)
        # ])
        # self.stem_other_modality = nn.Sequential(*[
        #     nn.Conv2d(in_channels, inplanes, kernel_size=3, stride=2, padding=1, bias=False),
        #     nn.SyncBatchNorm(inplanes),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(inplanes, inplanes, kernel_size=3, stride=1, padding=1, bias=False),
        ...
        #     nn.MaxPool2d(kernel_size=3, stride=2, padding=1)
        # ])
        # self.conv2_other_modality = nn.Sequential(*[
        #     nn.Conv2d(inplanes, 2 * inplanes, kernel_size=3, stride=2, padding=1, bias=False),
        #     nn.SyncBatchNorm(2 * inplanes),
        #     nn.ReLU(inplace=True)
        # ])
        # self.conv3_other_modality = nn.Sequential(*[
        #     nn.Conv2d(2 * inplanes, 4 * inplanes, kernel_size=3, stride=2, padding=1, bias=False),
        #     nn.SyncBatchNorm(4 * inplanes),
        #     nn.ReLU(inplace=True)
        # ])
        # self.conv4_other_modality = nn.Sequential(*[
        #     nn.Conv2d(4 * inplanes, 4 * inplanes, kernel_size=3, stride=2, padding=1, bias=False),
        #     nn.SyncBatchNorm(4 * inplanes),
        #     nn.ReLU(inplace=True)
        # ])
        # self.mono_conv=ModuleList([self.stem,self.conv2,self.conv3,self.conv4])
        ...
        # self.fc2 = nn.Conv2d(8 * inplanes, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        # self.fc2.apply(self._init_weights)

        # self.fc3 = nn.Conv2d(16 * inplanes, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        # self.fc3.apply(self._init_weights)

        # self.fc4 = nn.Conv2d(32 * inplanes, embed_dim, kernel_size=1, stride=1, padding=0, bias=True)
        # self.fc4.apply(self._init_weights)
    ...
    def forward(self, x,x_other_mod):
        ...
        def _inner_forward(x,x_other_mod):
            ...
            # c3 = self.conv3(c2)
            # c4 = self.conv4(c3)
            # c1_other_mod=self.stem_other_modality(x_other_mod)
            # c2_other_mod=self.conv2_other_modality(c1_other_mod)
            # c3_other_mod=self.conv3_other_modality(c2_other_mod)
            # c4_other_mod=self.conv4_other_modality(c3_other_mod)
            
            c1,c2,c3,c4=self.twin_conv(x,x_other_mod)
            # c1,c2,c3,c4=self.mono_conv(x)
            ...
            c1,c2,c3,c4=c1[:,:2*self.inplanes]+c1[:,2*self.inplanes:],c2[:,:4*self.inplanes]+c2[:,4*self.inplanes:],c3[:,8*self.inplanes:]+c3[:,:8*self.inplanes],c4[:,:16*self.inplanes]+c4[:,16*self.inplanes:]
            # c1,c2,c3,c4=c1+c1_other_mod,c2+c2_other_mod,c3+c3_other_mod,c4+c4_other_mod
            c1 = self.fc1(c1)
            c2 = self.fc2(c2)
            c3 = self.fc3(c3)
            c4 = self.fc4(c4)
