    ...
    def forward(self, query, reference_points, feat, feat_other_modalities, spatial_shapes, level_start_index):
        def _inner_forward(query, feat, feat_other_modalities):
            ...
            attn = self.attn(query_n, reference_points,
                                ...
                                level_start_index, None)
            # if self.num_mod>1:#>1
            feat_other_modalities = self.feat_norm(feat_other_modalities).type_as(feat_other_modalities)
            attn_other_modalities = self.attn(query_n, reference_points,
    ...
    def forward(self, query, query_other_modalities,reference_points, feat, spatial_shapes, level_start_index, H, W):
        ...
        def _inner_forward(query, query_other_modalities, feat):
            ...
            attn = self.attn(query_n, reference_points,
                             ...
                             level_start_index, None)
            # if self.num_mod>1:
            query_n_other_modalities = self.query_norm(query_other_modalities)
            attn_other_modalities_t = self.attn(query_n_other_modalities, reference_points,
                             feat_n, spatial_shapes,
                             level_start_index, None)
            query = query + attn
            ...
            if self.with_cffn:
                ...
                query_other_modalities = query_other_modalities + self.drop_path(self.ffn(self.ffn(query_other_modalities), H, W))
