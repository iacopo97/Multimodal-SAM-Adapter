...
class SpatialPriorModulewithGammaSpatial(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False, in_channels=3, num_mod=4):
        ...
        self.merger = nn.Conv2d(3*self.num_mod, self.num_mod, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            mod_selector_gamma2=F.interpolate(mod_selector_gamma2, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            mod_selector_gamma3=F.interpolate(mod_selector_gamma3, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            # mod_selector_gamma4_t=F.interpolate(mod_selector_gamma4_t, size=c1.shape[-2:], mode='bilinear', align_corners=False)
            mod_selector_t=torch.cat((mod_selector_gamma1,mod_selector_gamma2,mod_selector_gamma3),dim=1)
