...
class SpatialPriorModuleSparse(nn.Module):
    def __init__(self, inplanes=64, embed_dim=1024, in_ch=1,with_cp=False,D=2):
        super().__init__()
        self.with_cp = with_cp
        self.coordinates=dense_coordinates(torch.rand(1, in_ch, embed_dim,embed_dim).shape)
...
def main():
    ...
    datum=np.array(Image.open('/media/data4/sora/datasets/DELIVER_2/samples/lidar/training/cloud_MAP_1_point2_095300_lidar_front.png'),dtype=np.float32)
    # datum=np.array(Image.open('/media/data4/sora/datasets/DELIVER_2/samples/event/training/cloud_MAP_1_point2_095350_event_front.png'))
