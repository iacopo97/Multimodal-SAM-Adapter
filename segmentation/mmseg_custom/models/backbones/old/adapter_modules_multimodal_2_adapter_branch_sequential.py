...
DEBUG=True
if DEBUG:
    def visualize_maps(c1, c2, c3, c4):
        def visualize_single_map(c, title):
            bs, dim, _ = c.shape
            c = c.view(bs, dim, int(c.shape[2]**0.5), int(c.shape[2]**0.5))
            c = c[0].detach().cpu().numpy()
            fig, axes = plt.subplots(1, dim, figsize=(15, 15))
            fig.suptitle(title)
            for i in range(dim):
                axes[i].imshow(c[i], cmap='viridis')
                axes[i].axis('off')
            if not os.path.exists("DEBUG"):
                os.makedirs("DEBUG")
            plt.savefig(os.path.join("DEBUG", f"{title}.png"))

        visualize_single_map(c1, 'Map c1')
        visualize_single_map(c2, 'Map c2')
        visualize_single_map(c3, 'Map c3')
        visualize_single_map(c4, 'Map c4')

# Example usage:
# visualize_maps(c1, c2, c3, c4)
# if DEBUG:
