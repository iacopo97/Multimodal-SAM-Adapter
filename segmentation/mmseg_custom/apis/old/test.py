    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        ...
        if show or out_dir:
            ...
            for img, img_meta in zip(imgs, img_metas):
                ...
                if case is not None:
                    single_case, single_condition=check_case(img_meta['filename'],case, condition)
                if out_dir and case is not None:
                    # single_case, single_condition=check_case(img_meta['filename'],case)
                    out_file = osp.join(out_dir+"/prediction"+f"/{single_condition}"+f"/{single_case}", img_meta['ori_filename'])
                elif out_dir:
                ...
                # if 'overexposed' in img_meta['filename'] or 'underexposed' in img_meta['filename']:
                    if  case is not None and (single_case  !=  'ordinary') and mod_gamma_spatial is not None:
                        # modalities=[]
                        ...
                        save_mod_gamma_spatial_with_mean_and_variance_colormap(mod_gamma_spatial, img_meta, out_dir, single_case,single_condition)
                        # if nr_output==6:
        ...
        if pre_eval:
            ...
            if case is not None:
                ...
                #     result_dict['ordinary'].extend(result)
                #     ordinary_flag=False
                result_dict=fill_case_dictionary(result, single_case, single_condition, result_dict)
            else:
...
def multi_gpu_test(model,
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        with torch.no_grad():
            ...
            # result = model(return_loss=False, rescale=False, **data) ##MY MOD
            # if len(result)>1 and len(result)==6:
            #     nr_output=6
            #     result,mod_gamma_spatial_1,mod_gamma_spatial_2,mod_gamma_spatial_3,mod_gamma_spatial_4,mod_gamma_spatial_inj=result
            #     mod_gamma_spatial_1 = mod_gamma_spatial_1[0].transpose(1,2,0)
