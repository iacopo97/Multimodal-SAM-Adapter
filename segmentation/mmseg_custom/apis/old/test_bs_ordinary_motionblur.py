...
def fill_case_dictionary_motionblur_ordinary(result, single_case,single_condition, result_dict):
    # for c in case:
    ...
    # print(single_case)
    if single_case=='ordinary':
        result_dict[single_condition]["motionblur"].extend(result)
    else:
...
def single_gpu_test(model,
    ...
    if 'DELIVER' in dataset.__doc__:
        ...
        case=None
    if case is not None:
        result_dict=define_case_dictionary_ordinary_motionblur(case, condition)
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        with torch.no_grad():
            ...
            if (len(result)>1 and isinstance(result[0],list)) and bs==1: #len(result)>1
                result, mod_gamma_spatial, nr_output=reading_mod_gamma_spatial(result)
            if bs>1 and isinstance(result[0],list):#len(result[0])>1
                # mod_gamma_spatial=[]
                # for i in range(bs):
        ...
        if pre_eval:
            ...
            if case is not None:
                ...
                #     ordinary_flag=False
                    if bs>1: 
                        result_dict=fill_case_dictionary_motionblur_ordinary([result[count]], single_case[count], single_condition[count], result_dict)
                    else:
                        result_dict=fill_case_dictionary_motionblur_ordinary(result, single_case[count], single_condition[count], result_dict)
                    count +=1
            else:
...
def multi_gpu_test(model,
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        with torch.no_grad():
            ...
            #     result=[result]
            bs=data['img'][0].size(0)
            if (len(result)>1  and isinstance(result[0],list)) and bs==1:
                result, mod_gamma_spatial, nr_output=reading_mod_gamma_spatial(result)
            if bs>1 and (len(result[0])>1 and isinstance(result[0],list)):
                # mod_gamma_spatial=[]
                # for i in range(bs):
                # cum=result[i]+result[i+bs:]
                result, mod_gamma_spatial, nr_output=reading_mod_gamma_spatial_bs(result,bs)
